...
module AstTree (E : Epr.Epr) :
  ...
  let rec layout = function
    ...
    | And ps ->
        sprintf "(%s)" (List.inner_layout (List.map layout ps) "∧" "true")
    | Or ps ->
        sprintf "(%s)" (List.inner_layout (List.map layout ps) "∨" "true")
    | Not p -> "~" ^ layout p
  ...
  let neg_spec name ast =
    let rec aux = function
      ...
      | SpecApply (specname, args) ->
          if String.equal specname name then
            And
              [
                Not (SpecApply (specname, args));
                SpecApply ("co_" ^ specname, args);
              ]
  ...
  let vc_layout a =
    ...
    let rec aux indent = function
      | ForAll ff ->
          sprintf "%s%s" (mk_indent indent) (E.pretty_layout_forallformula ff)
      | Implies (SpecApply (n1, args1), SpecApply (n2, args2)) ->
          sprintf "%s(%s => %s)" (mk_indent indent)
            (aux 0 (SpecApply (n1, args1)))
            (aux 0 (SpecApply (n2, args2)))
      | Implies (p1, p2) ->
          sprintf "%s(%s => \n%s\n%s)" (mk_indent indent) (aux 0 p1)
            (aux (indent + 1) p2)
            (mk_indent indent)
      ...
      | Ite (p1, p2, p3) ->
          sprintf "%s(ite%s\n%s\n%s)" (mk_indent indent) (aux 1 p1)
            ...
            (aux (indent + 4) p3)
      | SpecApply (specname, args) ->
          sprintf "%s%s(%s)" (mk_indent indent) specname
            (List.to_string E.SE.layout args)
    in
    aux 0 a
  ...
  let layout_spec (args, formula) =
    sprintf "fun %s -> %s"
      (List.to_string T.layouttvar args)
  ...
  let eq a b =
    let rec aux a b =
      ...
      | Ite (p1, p2, p3), Ite (p1', p2', p3') ->
          aux p1 p1' && aux p2 p2' && aux p3 p3'
      | SpecApply (specname1, args1), SpecApply (specname2, args2) ->
          String.equal specname1 specname2 && List.for_all2 E.SE.eq args1 args2
      | _ -> false
    in
    aux a b
  ...
  let make_match (matched : Tp.Tp.tpedvar list) (target : Tp.Tp.tpedvar list)
    ...
    let handle_case matched_vars (body, matched_vars') =
      ...
      let eq_specs =
        List.filter_map
          ...
          (List.combine matched_vars matched_vars')
      in
      match body with None -> eq_specs | Some body -> body :: eq_specs
    in
  ...
  let get_app_args t specname =
    let rec aux t res =
      ...
      | SpecApply (specname', args) ->
          if String.equal specname specname' then args :: res else res
    in
    aux t []
  ...
  let get_uvars t =
    let rec aux t res =
      ...
      | SpecApply (_, args) ->
          List.filter_map
            (fun se ->
              match se with E.SE.Var (tp, name) -> Some (tp, name) | _ -> None)
            args
          @ res
    in
    List.filter (fun (tp, _) -> match tp with T.Int -> true | _ -> false)
    @@ List.remove_duplicates T.tpedvar_eq
    @@ aux t []
    ...
    let rec aux a =
      ...
      | And ps ->
          List.map
            ...
            (List.choose_list_list (List.map aux ps))
      | Implies (_, _) -> [ [ a ] ]
  ...
  let count_apps a names =
    ...
    let rec aux a =
      match a with
      | SpecApply (name, _) ->
          if List.exists (fun name' -> String.equal name name') names then
            c := !c + 1
          else ()
  ...
  let rec encode = function
    ...
    | Implies (p1, p2) ->
        encode_field "AImplies" (`List [ encode p1; encode p2 ])
  ...
  let spectable_decode = function
    | `List l ->
        List.fold_left
          ...
          StrMap.empty l
    | _ ->
        raise @@ InterExn (Printf.sprintf "%s::decode wrong type" "spec table")

  let spec_eq (args1, body1) (args2, body2) =
    if List.length args1 != List.length args2 then false
    else
      List.for_all
        (fun (arg1, arg2) -> T.tpedvar_eq arg1 arg2)
        (List.combine args1 args2)
      && E.eq_forallformula body1 body2
    ...
    StrMap.fold
      (fun name spec r ->
        if not r then false
          ...
          | Some spec' ->
              printf "%s not eq!\nspec 1:\n%s\nspec 2:\n%s\n" name
                (layout_spec spec) (layout_spec spec');
              false
          | _ ->
              printf "%s not eq!\nspec 1:\n%s\nspec 2:\n%s\n" name
                (layout_spec spec) "none";
              false)
      t1 true

  let spectable_eq t1 t2 = spectable_eq_succ t1 t2
  (* && (spectable_eq_succ t2 t1) *)
