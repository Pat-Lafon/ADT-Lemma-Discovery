module type Epr = sig
  ...
  val forallformula_to_z3 :
    Z3.context -> forallformula -> Solver.Z3aux.imp_version -> Z3.Expr.expr
...
module Epr (E : EprTree.EprTree) : Epr = struct
  ...
  let to_horns a =
    ...
    let rec disj ps =
      let rec aux ps =
        ...
        | [ _ ] ->
            raise
            @@ InterExn
                 (Printf.sprintf "to_horns:disj(%s)" (List.to_string layout ps))
        | h :: tl -> List.map (fun (pre, post) -> (neg h :: pre, post)) (aux tl)
      in
      aux ps
    and conj ps =
      let aux p =
        ...
        | _ ->
            raise
            @@ InterExn
                 (Printf.sprintf "to_horns:conj(%s)" (List.to_string layout ps))
      in
    ...
    and iteaux a =
      ...
      | Ite (p1, p2, p3) ->
          ...
          let p3s =
            List.map (fun (hd, tl) -> (hd @ [ neg p1 ], tl)) (iteaux p3)
          in
          p2s @ p3s
  ...
  let simplify_ite a =
    ...
    let simp_ite p1 p2 p3 =
      ...
      | x1, Not x2 -> if eq x1 x2 then Iff (p1, x1) else desugar_ite (p1, p2, p3)
    ...
    let rec aux a =
      ...
      | And ps ->
          And
            (List.filter_map
               (function True -> None | p -> Some p)
               (List.map aux ps))
      | Or ps ->
          let ps = List.map aux ps in
          if List.exists (function True -> true | _ -> false) ps then True
          else Or ps
      | Iff (p1, p2) ->
    ...
    let rec simplify_same = function
      | Implies (p1, Implies (p2, p3)) ->
          simplify_same (Implies (And [ p1; p2 ], p3))
