module type EprTree = sig
  ...
  val subst_forallformula :
    forallformula -> string list -> SE.t list -> forallformula
...
module EprTree (SE : SimpleExpr.SimpleExpr) :
  EprTree
  ...
  let forallformula_decode json =
    let e =
      InterExn (Printf.sprintf "%s::decode wrong type" forallformula_tpname)
    in
    let open Util in
  ...
  let rec layout = function
    ...
    | And ps ->
        sprintf "(%s)" (List.inner_layout (List.map layout ps) sym_and "true")
    | Or ps ->
        sprintf "(%s)" (List.inner_layout (List.map layout ps) sym_or "false")
  ...
  let pretty_layout indent e =
    ...
    let rec aux indent = function
      ...
      | Implies (Atom e1, Atom e2) ->
          sprintf "%s(%s %s %s)" (mk_indent indent) (aux 0 (Atom e1))
            sym_implies (aux 0 (Atom e2))
      | Implies (p1, p2) ->
          sprintf "%s(\n%s %s \n%s\n%s)" (mk_indent indent)
            (aux (indent + 1) p1)
            sym_implies
      ...
      | And [ p ] -> aux indent p
      | And [ Atom e1; Atom e2 ] ->
          sprintf "%s(%s %s %s)" (mk_indent indent) (aux 0 (Atom e1)) sym_and
            (aux 0 (Atom e2))
      | And [ Atom e1; Atom e2; Atom e3 ] ->
          sprintf "%s(%s %s %s %s %s)" (mk_indent indent) (aux 0 (Atom e1))
            sym_and (aux 0 (Atom e2)) sym_and (aux 0 (Atom e3))
      | And ps ->
          sprintf "%s(\n%s\n%s)" (mk_indent indent)
            (List.inner_layout
      ...
      | Or [ p ] -> aux indent p
      | Or [ Atom e1; Atom e2 ] ->
          sprintf "%s(%s %s %s)" (mk_indent indent) (aux 0 (Atom e1)) sym_or
            (aux 0 (Atom e2))
  ...
  let pretty_layout_forallformula (forallvars, body) =
    ...
    if List.length forallvars == 0 then layout body
    else
      sprintf "forall %s,%s"
        (List.inner_layout qvstr " " "")
        (pretty_layout 0 body)
  ...
  let layout_forallformula (forallvars, body) =
    ...
    if List.length forallvars == 0 then layout body
    else sprintf "forall %s,%s" (List.inner_layout qvstr " " "") (layout body)
  ...
  let prune body =
    let rec aux pos neg body =
      ...
      | Implies (_, _) | And _ | Or _ | Iff (_, _) ->
          raise @@ InterExn "ast:prune"
      | Ite (p1, p2, p3) -> (
  ...
  let substm m body =
    let args, argsvalue =
      StrMap.fold
        (fun name v (args, argsvalue) -> (name :: args, v :: argsvalue))
        m ([], [])
    in
