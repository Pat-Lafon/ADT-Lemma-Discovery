...
let add_docs_attrs docs attrs =
  let attrs =
    match docs.docs_pre with None -> attrs | Some ds -> docs_attr ds :: attrs
  in
  let attrs =
...
let add_info_attrs info attrs =
  match info with None -> attrs | Some ds -> attrs @ [ info_attr ds ]
...
let add_text_attrs dsl attrs = List.map text_attr dsl @ attrs
