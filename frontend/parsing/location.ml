  ...
  for pos = 0 to end_pos do
    ...
    | c ->
        if !line = !line_start && !line = !line_end then
          ...
          if pos < loc.loc_start.pos_cnum then Format.pp_print_char ppf '.'
          else Format.pp_print_char ppf c
        else if !line = !line_end then
          (* last line of multiline loc: print a dot for each char
             after loc_end, even whitespaces *)
          if pos < loc.loc_end.pos_cnum then Format.pp_print_char ppf c
...
let rec highlight_locations ppf locs =
  ...
  | Terminfo.Good_term -> (
      ...
      | Some lb -> (
          try
            highlight_terminfo ppf lb locs;
            true
          with Exit -> false))
...
let print_loc ppf loc =
  ...
  if file = "//toplevel//" then
    if highlight_locations ppf [ loc ] then ()
      ...
      fprintf ppf "Characters %i-%i" loc.loc_start.pos_cnum loc.loc_end.pos_cnum
  else (
...
let errorf ?(loc = none) ?(sub = []) ?(if_highlight = "") fmt =
  pp_ksprintf (fun msg -> { loc; msg; sub; if_highlight }) fmt
...
let error_of_exn exn =
  let rec loop = function
    | [] -> None
    | f :: rest -> ( match f exn with Some _ as r -> r | None -> loop rest)
  in
  loop !error_of_exn
...
let error_of_printer loc print x = errorf_prefixed ~loc "%a@?" print x
...
let () =
  register_error_of_exn (function
    | Sys_error msg ->
        Some (errorf_prefixed ~loc:(in_file !input_name) "I/O error: %s" msg)
    | Warnings.Errors n ->
        Some
