...
let needs_parens txt =
  is_infix (fixity_of_string txt) || List.mem txt.[0] prefix_symbols
...
class printer () =
  object (self : 'self)
    ...
    method longident f =
      ...
      | Lapply (y, s) -> pp f "%a(%a)" self#longident y self#longident s
    ...
    method constant f =
      ...
      | Pconst_integer (i, None) ->
          self#paren (i.[0] = '-') (fun f -> pp f "%s") f i
    ...
    (* trailing space*)
    method mutable_flag f =
      function Immutable -> () | Mutable -> pp f "mutable@;"

    method virtual_flag f =
      function Concrete -> () | Virtual -> pp f "virtual@;"
    ...
    method direction_flag f =
      function Upto -> pp f "to@ " | Downto -> pp f "downto@ "

    method private_flag f =
      function Public -> () | Private -> pp f "private@ "
    ...
    (* c ['a,'b] *)
    method class_params_def f =
      function
      | [] -> ()
      | l -> pp f "[%a] " (* space *) (self#list self#type_param ~sep:",") l
    ...
    method core_type f x =
      if x.ptyp_attributes <> [] then
        ...
        | Ptyp_alias (ct, s) -> pp f "@[<2>%a@;as@;'%s@]" self#core_type1 ct s
        | Ptyp_poly (sl, ct) ->
            pp f "@[<2>%a%a@]"
              (fun f l ->
                pp f "%a"
                  (fun f l ->
                    match l with
                    | [] -> ()
                    | _ -> pp f "%a@;.@;" (self#list self#tyvar ~sep:"@;") l)
                  l)
        ...
        | Ptyp_constr (li, l) ->
            pp f (* "%a%a@;" *) "%a%a"
              (fun f l ->
                ...
                | _ ->
                    self#list ~first:"(" ~last:")@;" self#core_type ~sep:"," f l)
              l self#longident_loc li
        | Ptyp_variant (l, closed, low) ->
            let type_variant_helper f x =
              match x with
              | Rtag (l, attrs, _, ctl) ->
                  pp f "@[<2>%a%a@;%a@]" self#string_quot l
                    (fun f l ->
                      match l with
                      | [] -> ()
                      | _ ->
                          pp f "@;of@;%a"
                            (self#list self#core_type ~sep:"&")
                            ctl)
                    ctl self#attributes attrs
              | Rinherit ct -> self#core_type f ct
            in
            pp f "@[<2>[%a%a]@]"
              (fun f l ->
                match l with
                ...
                | _ ->
                    pp f "%s@;%a"
                      ...
                      | Open, _ -> ">")
                      (self#list type_variant_helper ~sep:"@;<1 -2>| ")
                      l)
              l
              (fun f low ->
                match low with
                | Some [] | None -> ()
                | Some xs -> pp f ">@ %a" (self#list self#string_quot) xs)
              low
        | Ptyp_object (l, o) ->
            let core_field_type f (s, attrs, ct) =
              pp f "@[<hov2>%s%a@ :%a@ @]" s self#attributes attrs
                self#core_type ct
            in
    ...
    method pattern f x =
      let rec list_of_pattern acc = function
        (* only consider ((A|B)|C)*)
        | { ppat_desc = Ppat_or (p1, p2); _ } -> list_of_pattern (p2 :: acc) p1
        | x -> x :: acc
      in
      if x.ppat_attributes <> [] then
        pp f "((%a)%a)" self#pattern
          { x with ppat_attributes = [] }
          self#attributes x.ppat_attributes
      else
        match x.ppat_desc with
        | Ppat_alias (p, s) ->
            pp f "@[<2>%a@;as@;%a@]" self#pattern p protect_ident s.txt (* RA*)
        | Ppat_or (p1, p2) ->
            (* *)
            pp f "@[<hov0>%a@]"
              (self#list ~sep:"@,|" self#pattern)
              (list_of_pattern [] x)
        | _ -> self#pattern1 f x
    ...
    method pattern1 (f : Format.formatter) (x : pattern) : unit =
      ...
      if x.ppat_attributes <> [] then self#pattern f x
        ...
        | Ppat_variant (l, Some p) ->
            pp f "@[<2>`%s@;%a@]" l self#simple_pattern p
        | Ppat_construct ({ txt = Lident ("()" | "[]"); _ }, _) ->
            self#simple_pattern f x
    ...
    method simple_pattern (f : Format.formatter) (x : pattern) : unit =
      if x.ppat_attributes <> [] then self#pattern f x
        ...
        | Ppat_type li -> pp f "#%a" self#longident_loc li
        | Ppat_record (l, closed) -> (
            let longident_x_pattern f (li, p) =
              ...
              | _ ->
                  pp f "@[<2>%a@;=@;%a@]" self#longident_loc li self#pattern1 p
            in
            match closed with
            | Closed ->
                pp f "@[<2>{@;%a@;}@]"
                  (self#list longident_x_pattern ~sep:";@;")
                  l
            | _ ->
                pp f "@[<2>{@;%a;_}@]"
                  (self#list longident_x_pattern ~sep:";@;")
                  l)
        | Ppat_tuple l ->
            pp f "@[<1>(%a)@]" (self#list ~sep:"," self#pattern1) l (* level1*)
        | Ppat_constant c -> pp f "%a" self#constant c
        | Ppat_interval (c1, c2) ->
            pp f "%a..%a" self#constant c1 self#constant c2
        | Ppat_variant (l, None) -> pp f "`%s" l
        | Ppat_constraint (p, ct) ->
            pp f "@[<2>(%a@;:@;%a)@]" self#pattern1 p self#core_type ct
        | Ppat_lazy p -> pp f "@[<2>(lazy@;%a)@]" self#pattern1 p
        | Ppat_exception p -> pp f "@[<2>exception@;%a@]" self#pattern1 p
    ...
    method label_exp f (l, opt, p) =
      ...
      | Labelled l -> (
          match p.ppat_desc with
          | Ppat_var { txt; _ } when txt = l -> pp f "~%s@;" l
          ...
          when List.for_all (fun (lab, _) -> lab = Nolabel) args -> (
            ...
            | Ldot (path, (("get" | "set") as func)), a :: other_args -> (
                let print left right print_index indexes rem_args =
                  ...
                  | "get", [] ->
                      pp f "@[%a.%s%a%s@]" self#simple_expr a left
                        (self#list ~sep:"," print_index)
                        indexes right;
                      true
                  | "set", [ v ] ->
                      pp f "@[%a.%s%a%s@ <-@;<1 2>%a@]" self#simple_expr a left
                        (self#list ~sep:"," print_index)
                        indexes right self#simple_expr v;
                      true
                  | _ -> false
                in
                ...
                | ( Ldot (Lident "Bigarray", "Genarray"),
                    { pexp_desc = Pexp_array indexes; pexp_attributes = [] }
                    :: rest ) ->
    ...
    method expression f x =
      if x.pexp_attributes <> [] then
        pp f "((%a)@,%a)" self#expression
          { x with pexp_attributes = [] }
          self#attributes x.pexp_attributes
      else
        match x.pexp_desc with
        | Pexp_function _ | Pexp_fun _ | Pexp_match _ | Pexp_try _
        | Pexp_sequence _
          when pipe || semi ->
        ...
        | (Pexp_let _ | Pexp_letmodule _ | Pexp_open _) when semi ->
            self#paren true self#reset#expression f x
        | Pexp_fun (l, e0, p, e) ->
            pp f "@[<2>fun@;%a@;->@;%a@]" self#label_exp (l, e0, p)
              self#expression e
        | Pexp_function l -> pp f "@[<hv>function%a@]" self#case_list l
        | Pexp_match (e, l) ->
        ...
        | Pexp_let (rf, l, e) ->
            ...
            (*   self#rec_flag rf *)
            pp f "@[<2>%a in@;<1 -2>%a@]" self#reset#bindings (rf, l)
              self#expression e
        | Pexp_apply (e, l) -> (
              ...
              | `Infix s -> (
                  ...
                  | _ ->
                      pp f "@[<2>%a %a@]" self#simple_expr e
                        (self#list self#label_x_expression_param)
                        l)
              | `Prefix s -> (
                  let s =
                    if List.mem s [ "~+"; "~-"; "~+."; "~-." ] then
                  ...
                  pp f "@[<hov2>%a@]"
                    (fun f (e, l) ->
                      pp f "%a@ %a" self#expression2 e
                        (self#list self#reset#label_x_expression_param)
                        l)
        ...
        | Pexp_setfield (e1, li, e2) ->
            pp f "@[<2>%a.%a@ <-@ %a@]" self#simple_expr e1 self#longident_loc
              li self#expression e2
        | Pexp_ifthenelse (e1, e2, eo) ->
            ...
            pp f fmt self#under_ifthenelse#expression e1
              ...
              (fun f eo ->
                match eo with
                | Some x ->
                    pp f "@;@[<2>else@;%a@]" self#under_semi#expression x
                | None -> () (* pp f "()" *))
              eo
        | Pexp_sequence _ ->
            ...
            pp f "@[<hv>%a@]"
              (self#list self#under_semi#expression ~sep:";@;")
              lst
        | Pexp_new li -> pp f "@[<hov2>new@ %a@]" self#longident_loc li
        | Pexp_setinstvar (s, e) ->
        ...
        | Pexp_override l ->
            ...
            let string_x_expression f (s, e) =
              pp f "@[<hov2>%s@ =@ %a@]" s.txt self#expression e
            in
            pp f "@[<hov2>{<%a>}@]" (self#list string_x_expression ~sep:";") l
        | Pexp_letmodule (s, me, e) ->
            pp f "@[<hov2>let@ module@ %s@ =@ %a@ in@ %a@]" s.txt
              self#reset#module_expr me self#expression e
        | Pexp_assert e -> pp f "@[<hov2>assert@ %a@]" self#simple_expr e
        | Pexp_lazy e -> pp f "@[<hov2>lazy@ %a@]" self#simple_expr e
        (* Pexp_poly: impossible but we should print it anyway, rather
           than assert false *)
        | Pexp_poly (e, None) -> pp f "@[<hov2>!poly!@ %a@]" self#simple_expr e
        | Pexp_poly (e, Some ct) ->
            pp f "@[<hov2>(!poly!@ %a@ : %a)@]" self#simple_expr e
              self#core_type ct
        | Pexp_open (ovf, lid, e) ->
            pp f "@[<2>let open%s %a in@;%a@]" (override ovf) self#longident_loc
              lid self#expression e
    ...
    method simple_expr f x =
      if x.pexp_attributes <> [] then self#expression f x
        ...
        | Pexp_pack me -> pp f "(module@;%a)" self#module_expr me
        | Pexp_newtype (lid, e) ->
        ...
        | Pexp_record (l, eo) ->
            ...
            pp f "@[<hv0>@[<hv2>{@;%a%a@]@;}@]" (* "@[<hov2>{%a%a}@]" *)
              (self#option ~last:" with@;" self#simple_expr)
              eo
              (self#list longident_x_expression ~sep:";@;")
              l
        | Pexp_array l ->
            pp f "@[<0>@[<2>[|%a|]@]@]"
              (self#list self#under_semi#simple_expr ~sep:";")
              l
        | Pexp_while (e1, e2) ->
    ...
    method attributes f l = List.iter (self#attribute f) l
    method item_attributes f l = List.iter (self#item_attribute f) l
    method attribute f (s, e) = pp f "@[<2>[@@%s@ %a]@]" s.txt self#payload e
    ...
    method value_description f x =
      ...
      pp f "@[<hov2>%a%a@]" self#core_type x.pval_type
        (fun f x ->
          if x.pval_prim <> [] then
            pp f "@ =@ %a" (self#list self#constant_string) x.pval_prim)
        x

    method extension f (s, e) = pp f "@[<2>[%%%s@ %a]@]" s.txt self#payload e
    ...
    method class_signature f { pcsig_self = ct; pcsig_fields = l; _ } =
      let class_type_field f x =
        ...
        | Pctf_inherit ct ->
            pp f "@[<2>inherit@ %a@]%a" self#class_type ct self#item_attributes
              x.pctf_attributes
        | Pctf_val (s, mf, vf, ct) ->
            pp f "@[<2>val @ %a%a%s@ :@ %a@]%a" self#mutable_flag mf
              self#virtual_flag vf s self#core_type ct self#item_attributes
              x.pctf_attributes
        | Pctf_method (s, pf, vf, ct) ->
            pp f "@[<2>method %a %a%s :@;%a@]%a" self#private_flag pf
              self#virtual_flag vf s self#core_type ct self#item_attributes
              x.pctf_attributes
        | Pctf_constraint (ct1, ct2) ->
            pp f "@[<2>constraint@ %a@ =@ %a@]%a" self#core_type ct1
              self#core_type ct2 self#item_attributes x.pctf_attributes
      ...
      | Pcty_constr (li, l) ->
          pp f "%a%a%a"
            (fun f l ->
              match l with
              | [] -> ()
              | _ -> pp f "[%a]@ " (self#list self#core_type ~sep:",") l)
            l self#longident_loc li self#attributes x.pcty_attributes
      | Pcty_arrow (l, co, cl) ->
          pp f "@[<2>%a@;->@;%a@]" (* FIXME remove parens later *)
    ...
    method class_type_declaration_list f l =
      let class_type_declaration kwd f
          ({ pci_params = ls; pci_name = { txt; _ }; _ } as x) =
        pp f "@[<2>%s %a%a%s@ =@ %a@]%a" kwd self#virtual_flag x.pci_virt
          self#class_params_def ls txt self#class_type x.pci_expr
      ...
      | x :: xs ->
          pp f "@[<v>%a@,%a@]"
            ...
            x
            (self#list ~sep:"@," (class_type_declaration "and"))
            xs

    method class_field f x =
      match x.pcf_desc with
      | Pcf_inherit (ovf, ce, so) ->
          pp f "@[<2>inherit@ %s@ %a%a@]%a" (override ovf) self#class_expr ce
            (fun f so ->
              match so with None -> () | Some s -> pp f "@ as %s" s)
            so self#item_attributes x.pcf_attributes
      | Pcf_val (s, mf, Cfk_concrete (ovf, e)) ->
          pp f "@[<2>val%s %a%s =@;%a@]%a" (override ovf) self#mutable_flag mf
            s.txt self#expression e self#item_attributes x.pcf_attributes
      | Pcf_method (s, pf, Cfk_virtual ct) ->
          pp f "@[<2>method virtual %a %s :@;%a@]%a" self#private_flag pf s.txt
            self#core_type ct self#item_attributes x.pcf_attributes
      | Pcf_val (s, mf, Cfk_virtual ct) ->
          pp f "@[<2>val virtual %a%s :@ %a@]%a" self#mutable_flag mf s.txt
            self#core_type ct self#item_attributes x.pcf_attributes
      | Pcf_method (s, pf, Cfk_concrete (ovf, e)) ->
          ...
          in
          pp f "@[<2>method%s %a%a@]%a" (override ovf) self#private_flag pf
            (fun f e ->
              match e.pexp_desc with
              | Pexp_poly (e, Some ct) ->
                  pp f "%s :@;%a=@;%a" s.txt self#core_type ct self#expression e
              | Pexp_poly (e, None) -> bind e
              | _ -> bind e)
            e self#item_attributes x.pcf_attributes
      | Pcf_constraint (ct1, ct2) ->
          pp f "@[<2>constraint %a =@;%a@]%a" self#core_type ct1 self#core_type
            ct2 self#item_attributes x.pcf_attributes
      | Pcf_initializer e ->
          pp f "@[<2>initializer@ %a@]%a" self#expression e self#item_attributes
            x.pcf_attributes
    ...
    method class_structure f { pcstr_self = p; pcstr_fields = l } =
      pp f "@[<hv0>@[<hv2>object%a@;%a@]@;end@]"
        (fun f p ->
          ...
          | _ -> pp f " (%a)" self#pattern p)
        p
        (self#list self#class_field)
        l

    method class_expr f x =
      if x.pcl_attributes <> [] then
        pp f "((%a)%a)" self#class_expr
        ...
        | Pcl_let (rf, l, ce) ->
            pp f "%a@ in@ %a" self#bindings (rf, l) self#class_expr ce
        | Pcl_apply (ce, l) ->
            pp f "(%a@ %a)" self#class_expr ce
              (self#list self#label_x_expression_param)
              l
        | Pcl_constr (li, l) ->
            pp f "%a%a"
              (fun f l ->
                if l <> [] then
                  pp f "[%a]@ " (self#list self#core_type ~sep:",") l)
              l self#longident_loc li
        | Pcl_constraint (ce, ct) ->
            pp f "(%a@ :@ %a)" self#class_expr ce self#class_type ct
        | Pcl_extension e -> self#extension f e

    method module_type f x =
      if x.pmty_attributes <> [] then
        pp f "((%a)%a)" self#module_type
          { x with pmty_attributes = [] }
          self#attributes x.pmty_attributes
      else
        match x.pmty_desc with
        | Pmty_ident li -> pp f "%a" self#longident_loc li
        | Pmty_alias li -> pp f "(module %a)" self#longident_loc li
        | Pmty_signature s ->
        ...
        | Pmty_functor (s, Some mt1, mt2) ->
            if s.txt = "_" then
              pp f "@[<hov2>%a@ ->@ %a@]" self#module_type mt1 self#module_type
                mt2
            else
        ...
        | Pmty_with (mt, l) -> (
            let with_constraint f = function
              ...
              | Pwith_module (li, li2) ->
                  pp f "module %a =@ %a" self#longident_loc li
                    self#longident_loc li2
              | Pwith_typesubst ({ ptype_params = ls; _ } as td) ->
                  let ls = List.map fst ls in
                  pp f "type@ %a %s :=@ %a"
                    (self#list self#core_type ~sep:"," ~first:"(" ~last:")")
                    ls td.ptype_name.txt self#type_declaration td
              | Pwith_modsubst (s, li2) ->
                  pp f "module %s :=@ %a" s.txt self#longident_loc li2
            in
            match l with
            | [] -> pp f "@[<hov2>%a@]" self#module_type mt
            | _ ->
                pp f "@[<hov2>(%a@ with@ %a)@]" self#module_type mt
                  (self#list with_constraint ~sep:"@ and@ ")
                  l)
        | Pmty_typeof me ->
            pp f "@[<hov2>module@ type@ of@ %a@]" self#module_expr me
        | Pmty_extension e -> self#extension f e

    method signature f x = self#list ~sep:"@\n" self#signature_item f x

    method signature_item f x : unit =
      match x.psig_desc with
      | Psig_type (rf, l) -> self#type_def_list f (rf, l)
      | Psig_value vd ->
          let intro = if vd.pval_prim = [] then "val" else "external" in
          pp f "@[<2>%s@ %a@ :@ %a@]%a" intro protect_ident vd.pval_name.txt
            self#value_description vd self#item_attributes vd.pval_attributes
      | Psig_typext te -> self#type_extension f te
      | Psig_exception ed -> self#exception_declaration f ed
      | Psig_class l -> (
          ...
          | x :: xs ->
              pp f "@[<v>%a@,%a@]"
                ...
                x
                (self#list ~sep:"@," (class_description "and"))
                xs)
      | Psig_module ({ pmd_type = { pmty_desc = Pmty_alias alias }; _ } as pmd)
        ->
          pp f "@[<hov>module@ %s@ =@ %a@]%a" pmd.pmd_name.txt
            self#longident_loc alias self#item_attributes pmd.pmd_attributes
      | Psig_module pmd ->
          pp f "@[<hov>module@ %s@ :@ %a@]%a" pmd.pmd_name.txt self#module_type
            pmd.pmd_type self#item_attributes pmd.pmd_attributes
      | Psig_open od ->
          pp f "@[<hov2>open%s@ %a@]%a"
            (override od.popen_override)
            self#longident_loc od.popen_lid self#item_attributes
            od.popen_attributes
      | Psig_include incl ->
          pp f "@[<hov2>include@ %a@]%a" self#module_type incl.pincl_mod
      ...
      | Psig_modtype { pmtd_name = s; pmtd_type = md; pmtd_attributes = attrs }
        ->
          pp f "@[<hov2>module@ type@ %s%a@]%a" s.txt
            ...
            md self#item_attributes attrs
      | Psig_class_type l -> self#class_type_declaration_list f l
            ...
            | pmd :: tl ->
                if not first then
                  pp f "@ @[<hov2>and@ %s:@ %a@]%a" pmd.pmd_name.txt
                    self#module_type pmd.pmd_type self#item_attributes
                    pmd.pmd_attributes
                else
    ...
    method module_expr f x =
      if x.pmod_attributes <> [] then
        ...
        | Pmod_constraint (me, mt) ->
            pp f "@[<hov2>(%a@ :@ %a)@]" self#module_expr me self#module_type mt
        | Pmod_ident li -> pp f "%a" self#longident_loc li
        | Pmod_functor (_, None, me) ->
            pp f "functor ()@;->@;%a" self#module_expr me
        | Pmod_functor (s, Some mt, me) ->
            pp f "functor@ (%s@ :@ %a)@;->@;%a" s.txt self#module_type mt
              self#module_expr me
        | Pmod_apply (me1, me2) ->
            pp f "%a(%a)" self#module_expr me1 self#module_expr me2
        | Pmod_unpack e -> pp f "(val@ %a)" self#expression e
    ...
    method payload f =
      ...
      | PStr [ { pstr_desc = Pstr_eval (e, attrs) } ] ->
          pp f "@[<2>%a@]%a" self#expression e self#item_attributes attrs
    ...
    method binding f { pvb_pat = p; pvb_expr = x; _ } =
        ...
        if x.pexp_attributes <> [] then pp f "=@;%a" self#expression x
          ...
          | Pexp_fun (label, eo, p, e) ->
              if label = Nolabel then
                ...
                pp f "%a@ %a" self#label_exp (label, eo, p)
      ...
      if x.pexp_attributes <> [] then
        ...
        | _, Ppat_constraint (p, ty) -> (
            (* special case for the first*)
            match ty.ptyp_desc with
            | Ptyp_poly _ ->
                pp f "%a@;:@;%a@;=@;%a" self#simple_pattern p self#core_type ty
                  self#expression x
            | _ ->
                pp f "(%a@;:@;%a)@;=@;%a" self#simple_pattern p self#core_type
                  ty self#expression x)
        ...
        | _ -> pp f "%a@;=@;%a" self#pattern p self#expression x

    (* [in] is not printed *)
    method bindings f (rf, l) =
      let binding kwd rf f x =
        pp f "@[<2>%s %a%a@]@ %a" kwd self#rec_flag rf self#binding x
          self#item_attributes x.pvb_attributes
      in
      match l with
      | [] -> ()
      ...
      | x :: xs ->
          pp f "@[<v>%a@,%a@]" (binding "let" rf) x
            (self#list ~sep:"@," (binding "and" Nonrecursive))
            xs

    method structure_item f x =
      match x.pstr_desc with
      | Pstr_eval (e, attrs) ->
          pp f "@[<hov2>;;%a@]%a" self#expression e self#item_attributes attrs
            ...
            (fun f me ->
              ...
              | Pmod_constraint
                ...
                when me.pmod_attributes = [] ->
                  pp f " :@;%a@;=@;%a@;" self#module_type mt self#module_expr
                    me'
              | _ -> pp f " =@ %a" self#module_expr me)
            x.pmb_expr self#item_attributes x.pmb_attributes
      | Pstr_open od ->
          pp f "@[<2>open%s@;%a@]%a"
            (override od.popen_override)
            self#longident_loc od.popen_lid self#item_attributes
      ...
      | Pstr_class l -> (
          ...
          let class_declaration kwd f
            ...
            pp f "@[<2>%s %a%a%s %a%a=@;%a@]%a" kwd self#virtual_flag x.pci_virt
              self#class_params_def ls txt (self#list self#label_exp) args
              (self#option class_constraint)
          ...
          | x :: xs ->
              pp f "@[<v>%a@,%a@]"
                ...
                x
                (self#list ~sep:"@," (class_declaration "and"))
                xs)
      | Pstr_class_type l -> self#class_type_declaration_list f l
      | Pstr_primitive vd ->
            ...
            | { pmb_expr = { pmod_desc = Pmod_constraint (expr, typ) } } as pmb
              ->
                pp f "@[<hov2>@ and@ %s:%a@ =@ %a@]%a" pmb.pmb_name.txt
                  self#module_type typ self#module_expr expr
          ...
          | ({ pmb_expr = { pmod_desc = Pmod_constraint (expr, typ) } } as pmb)
            :: l2 ->
              pp f "@[<hv>@[<hov2>module@ rec@ %s:%a@ =@ %a@]%a@ %a@]"
                pmb.pmb_name.txt self#module_type typ self#module_expr expr
                self#item_attributes pmb.pmb_attributes
    ...
    method type_params f =
      ...
      | [] -> ()
      | l ->
          pp f "%a " (self#list self#type_param ~first:"(" ~last:")" ~sep:",") l
    ...
    method type_def_list f (rf, l) =
      let type_decl kwd rf f x =
        ...
        in
        pp f "@[<2>%s %a%a%s%s%a@]%a" kwd self#nonrec_flag rf self#type_params
          x.ptype_params x.ptype_name.txt eq self#type_declaration x
      ...
      | x :: xs ->
          pp f "@[<v>%a@,%a@]" (type_decl "type" rf) x
            (self#list ~sep:"@," (type_decl "and" Recursive))
            xs

    method record_declaration f lbls =
      let type_record_field f pld =
        pp f "@[<2>%a%s:@;%a@;%a@]" self#mutable_flag pld.pld_mutable
          pld.pld_name.txt self#core_type pld.pld_type self#attributes
          pld.pld_attributes
      in
    ...
    method type_declaration f x =
      ...
      let manifest f =
        ...
        | Some y ->
            ...
            else pp f "@;%a" self#core_type y
      in
      let constructor_declaration f pcd =
        pp f "|@;";
        self#constructor_declaration f
      ...
      let repr f =
        ...
        | Ptype_variant xs ->
            pp f "%t%t@\n%a" intro priv
              ...
              xs
        | Ptype_abstract -> ()
        | Ptype_record l -> pp f "%t%t@;%a" intro priv self#record_declaration l
        | Ptype_open -> pp f "%t%t@;.." intro priv
      in
      let constraints f =
        List.iter
          (fun (ct1, ct2, _) ->
            pp f "@[<hov2>@ constraint@ %a@ =@ %a@]" self#core_type ct1
              self#core_type ct2)
          x.ptype_cstrs
      in
    ...
    method type_extension f x =
      ...
      pp f "@[<2>type %a%a +=%a@]%a"
        (fun f -> function
          ...
          | l ->
              pp f "%a@;"
                (self#list self#type_param ~first:"(" ~last:")" ~sep:",")
                l)
        x.ptyext_params self#longident_loc x.ptyext_path
        (self#list ~sep:"" extension_constructor)
        x.ptyext_constructors self#item_attributes x.ptyext_attributes
    ...
    method extension_constructor f x =
      ...
      | Pext_decl (l, r) ->
          self#constructor_declaration f
            (x.pext_name.txt, l, r, x.pext_attributes)
      | Pext_rebind li ->
          pp f "%s%a@;=@;%a" x.pext_name.txt self#attributes x.pext_attributes
            self#longident_loc li

    method case_list f l : unit =
      let aux f { pc_lhs; pc_guard; pc_rhs } =
        pp f "@;| @[<2>%a%a@;->@;%a@]" self#pattern pc_lhs
          (self#option self#expression ~first:"@;when@;")
    ...
    method label_x_expression_param f (l, e) =
      ...
      in
      match l with
      | Nolabel -> self#expression2 f e
      (* level 2*)
      | Optional str ->
          if Some str = simple_name then pp f "?%s" str
          else pp f "?%s:%a" str self#simple_expr e
      | Labelled lbl ->
          if Some lbl = simple_name then pp f "~%s" lbl
          else pp f "~%s:%a" lbl self#simple_expr e
...
let toplevel_phrase f x =
  ...
  (* pp_close_box f (); *)
  | Ptop_dir (s, da) -> pp f "@[<hov2>#%s@ %a@]" s default#directive_argument da
(* pp f "@[<hov2>#%s@ %a@]" s directive_argument da *)

let expression f x = pp f "@[%a@]" default#expression x
