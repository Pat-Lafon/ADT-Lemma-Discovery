...
module MakeMap (Map : MapArgument) = struct
  ...
  and map_bindings rec_flag list = List.map map_binding list
  ...
  and map_structure_item item =
    ...
    let str_desc =
      ...
      | Tstr_class list ->
          let list =
            List.map
              (fun (ci, string_list) -> (map_class_declaration ci, string_list))
              list
          in
          Tstr_class list
  ...
  and map_type_declaration decl =
    ...
    let typ_kind =
      ...
      | Ttype_record list ->
          let list =
            List.map
              (fun ld -> { ld with ld_type = map_core_type ld.ld_type })
              list
          in
          Ttype_record list
  ...
  and map_pattern pat =
    ...
    let pat_desc =
      ...
      | Tpat_variant (label, pato, rowo) ->
          let pato =
            match pato with None -> pato | Some pat -> Some (map_pattern pat)
          in
          Tpat_variant (label, pato, rowo)
  ...
  and map_expression exp =
    ...
    let exp_desc =
      ...
      | Texp_let (rec_flag, list, exp) ->
          Texp_let (rec_flag, map_bindings rec_flag list, map_expression exp)
      ...
      | Texp_record (list, expo) ->
          let list =
            List.map
              (fun (lid, lab_desc, exp) -> (lid, lab_desc, map_expression exp))
              list
          in
          let expo =
            match expo with
      ...
      | Texp_override (path, list) ->
          Texp_override
            ( path,
              List.map
                (fun (path, lid, exp) -> (path, lid, map_expression exp))
                list )
    ...
    let mty_desc =
      ...
      | Tmty_functor (id, name, mtype1, mtype2) ->
          Tmty_functor
            ( id,
              name,
              Misc.may_map map_module_type mtype1,
  ...
  and map_class_expr cexpr =
    ...
    let cl_desc =
      ...
      | Tcl_constraint (cl, None, string_list1, string_list2, concr) ->
          Tcl_constraint
            (map_class_expr cl, None, string_list1, string_list2, concr)
      ...
      | Tcl_fun (label, pat, priv, cl, partial) ->
          Tcl_fun
            ( label,
              map_pattern pat,
              List.map
                (fun (id, name, exp) -> (id, name, map_expression exp))
                priv,
              map_class_expr cl,
              partial )
      | Tcl_apply (cl, args) ->
          Tcl_apply
            ( map_class_expr cl,
              List.map
                (fun (label, expo) -> (label, may_map map_expression expo))
                args )
      | Tcl_let (rec_flat, bindings, ivars, cl) ->
          Tcl_let
            ( rec_flat,
              map_bindings rec_flat bindings,
              List.map
                (fun (id, name, exp) -> (id, name, map_expression exp))
                ivars,
              map_class_expr cl )
      | Tcl_constraint (cl, Some clty, vals, meths, concrs) ->
          Tcl_constraint
            (map_class_expr cl, Some (map_class_type clty), vals, meths, concrs)
