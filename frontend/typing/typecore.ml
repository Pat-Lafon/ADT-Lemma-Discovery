...
let type_module =
  ref
    (fun env md -> assert false
      : Env.t -> Parsetree.module_expr -> Typedtree.module_expr)

(* Forward declaration, to be filled in by Typemod.type_open *)

let type_open = ref (fun _ -> assert false)
...
let unify_exp_types loc env ty expected_ty =
  ...
  | Unify trace -> raise (Error (loc, env, Expr_type_clash trace))
  | Tags (l1, l2) ->
      raise (Typetexp.Error (loc, env, Typetexp.Variant_tags (l1, l2)))
...
let has_variants p =
  try
    iter_pattern
      (function { pat_desc = Tpat_variant _ } -> raise Exit | _ -> ())
      p;
    false
  with Exit -> true
...
let enter_orpat_variables loc env p1_vs p2_vs =
  ...
  let rec unify_vars p1_vs p2_vs =
    ...
    | (x1, t1, _, l1, a1) :: rem1, (x2, t2, _, l2, a2) :: rem2
...
let build_or_pat env loc lid =
  let path, decl = Typetexp.find_type env loc lid in
...
struct
  ...
  let ambiguous_types env lbl others =
    ...
    let others = List.map (fun (lbl, _) -> get_type_path env lbl) others in
              ...
              (if in_env lbl then
                 ...
                 warn lid.loc
                   (Warnings.Name_out_of_scope
                      (s, [ Longident.last lid.txt ], false)));
...
let disambiguate_lid_a_list loc closed env opath lid_a_list =
  ...
  (if !w_pr then
     ...
     | (_, types) :: others as amb ->
         let paths =
           List.map (fun (_, lbl, _) -> Label.get_type_path env lbl) lbl_a_list
         in
...
let rec type_pat ~constrs ~labels ~no_existentials ~mode ~explode ~env sp
  ...
  | Ppat_lazy sp1 ->
      let nv = newvar () in
      unify_pat_types loc !env
        (instance_def (Predef.type_lazy_t nv))
        expected_ty;
...
let partial_pred ~lev ?mode ?explode env expected_ty constrs labels p =
  ...
  try
    ...
    let typed_p =
      type_pat ~allow_existentials:true ~lev ~constrs ~labels ?mode ?explode env
        p expected_ty
    in
    set_state state env;
  ...
  let pv, met_env =
    List.fold_right
      (fun (id, ty, name, loc, as_var) (pv, env) ->
        let check s =
          if as_var then Warnings.Unused_var s else Warnings.Unused_var_strict s
        in
...
let iter_ppat f p =
  ...
  | Ppat_exception p | Ppat_alias (p, _) | Ppat_constraint (p, _) | Ppat_lazy p
    ->
      f p
  ...
  let rec loop p =
    ...
    | Ppat_construct (lid, _) ->
        (try
           ...
           List.iter
             (fun (cstr, _) -> if cstr.cstr_generalized then raise Exit)
             cstrs
         with Not_found -> ());
        iter_ppat loop p
    | _ -> iter_ppat loop p
...
let duplicate_ident_types loc caselist env =
  let caselist =
    List.filter (fun { pc_lhs } -> contains_gadt env pc_lhs) caselist
  in
...
and type_expect_ ?in_function ?(recarg = Rejected) env sexp ty_expected =
  ...
  | Pexp_match (sarg, caselist) ->
      ...
      let val_cases, partial =
        type_cases env arg.exp_type ty_expected true loc val_caselist
      in
      let exn_cases, _ =
        type_cases env Predef.type_exn ty_expected false loc exn_caselist
      in
      re
        {
        ...
        | None -> (
            ...
            | Some exp -> (
                ...
                | Some (_, p', _) as op ->
                    ...
                    begin_def ();
                    let ty =
                      newconstr p' (instance_list env decl.type_params)
                    in
                    end_def ();
                    generalize_structure ty;
                    (ty, op)))
  ...
  | Pexp_setinstvar (lab, snewval) -> (
      try
        ...
        | Val_ivar _ ->
            raise
              (Error (loc, env, Instance_variable_not_mutable (true, lab.txt)))
        | _ ->
            raise
              (Error (loc, env, Instance_variable_not_mutable (false, lab.txt)))
      with Not_found ->
        ...
        raise
          (Error (loc, env, Unbound_instance_variable (lab.txt, valid_vars))))
  | Pexp_override lst -> (
      let _ =
        List.fold_right
          (fun (lab, _) l ->
  ...
  let ty_arg, ty_res =
    ...
    with Unify _ -> (
      ...
      | _ ->
          raise
            (Error
               (loc_fun, env, Too_many_arguments (in_function <> None, ty_fun))))
  in
  let ty_arg =
    ...
    CamlinternalFormatBasics.(
      CamlinternalFormat.(
        let mk_exp_loc pexp_desc =
          { pexp_desc; pexp_loc = loc; pexp_attributes = [] }
        ...
        and mk_iconv iconv =
          ...
          | _ -> raise @@ Failure "zz: typecore::mk_iconv"
        and mk_fconv (f_flag_conv, f_kind_fconv) =
          match (f_flag_conv, f_kind_fconv) with
          | Float_flag_, Float_f -> mk_constr "Float_f" []
          | Float_flag_p, Float_f -> mk_constr "Float_pf" []
          | Float_flag_s, Float_f -> mk_constr "Float_sf" []
          | Float_flag_, Float_e -> mk_constr "Float_e" []
          | Float_flag_p, Float_e -> mk_constr "Float_pe" []
          | Float_flag_s, Float_e -> mk_constr "Float_se" []
          | Float_flag_, Float_E -> mk_constr "Float_E" []
          | Float_flag_p, Float_E -> mk_constr "Float_pE" []
          | Float_flag_s, Float_E -> mk_constr "Float_sE" []
          | Float_flag_, Float_g -> mk_constr "Float_g" []
          | Float_flag_p, Float_g -> mk_constr "Float_pg" []
          | Float_flag_s, Float_g -> mk_constr "Float_sg" []
          | Float_flag_, Float_G -> mk_constr "Float_G" []
          | Float_flag_p, Float_G -> mk_constr "Float_pG" []
          | Float_flag_s, Float_G -> mk_constr "Float_sG" []
          | Float_flag_, Float_h -> mk_constr "Float_h" []
          | Float_flag_p, Float_h -> mk_constr "Float_ph" []
          | Float_flag_s, Float_h -> mk_constr "Float_sh" []
          | Float_flag_, Float_H -> mk_constr "Float_H" []
          | Float_flag_p, Float_H -> mk_constr "Float_pH" []
          | Float_flag_s, Float_H -> mk_constr "Float_sH" []
          | _, Float_F -> mk_constr "Float_F" []
          | _, Float_CF -> mk_constr "Float_CF" []
        and mk_counter cnt =
          match cnt with
        ...
        if args = [] then texp
          ...
          let func texp =
            ...
            in
            {
              texp with
              exp_type = ty_fun;
              exp_desc = Texp_function (Nolabel, [ case eta_pat e ], Total);
            }
          in
          ...
          if is_nonexpansive texp then func texp
          else
...
and type_application env funct sargs =
  ...
  let rec type_unknown_args
      (args : (Asttypes.arg_label * (unit -> Typedtree.expression) option) list)
    ...
    | (l1, sarg1) :: sargl ->
        let ty1, ty2 =
          ...
          | Tvar _ ->
              ...
              let not_identity = function
                | Texp_ident
                    ( _,
                      ...
                      {
                        val_kind =
                          Val_prim { Primitive.prim_name = "%identity" };
                      } ) ->
                    false
                | _ -> true
              in
              ...
              unify env ty_fun
                (newty (Tarrow (l1, t1, t2, Clink (ref Cunknown))));
              (t1, t2)
          | Tarrow (l, t1, t2, _)
            when l = l1
          ...
          | td -> (
              ...
              | Tarrow _ ->
                  if !Clflags.classic || not (has_label l1 ty_fun) then
                    raise
                      (Error
                         (sarg1.pexp_loc, env, Apply_wrong_label (l1, ty_res)))
                  else
  ...
  let rec type_args args omitted ty_fun ty_fun0 ty_old sargs more_sargs =
    ...
    | ( ({ desc = Tarrow (l, ty, ty_fun, com); level = lv } as ty_fun'),
      ...
      when (sargs <> [] || more_sargs <> []) && commu_repr com = Cok ->
        ...
        let sargs, more_sargs, arg =
          if ignore_labels && not (is_optional l) then
            (* In classic mode, omitted = [] *)
            match (sargs, more_sargs) with
            | (l', sarg0) :: _, _ ->
                raise
                  (Error (sarg0.pexp_loc, env, Apply_wrong_label (l', ty_old)))
            | _, (l', sarg0) :: more_sargs ->
                if l <> l' && l' <> Nolabel then
                  raise
                    (Error (sarg0.pexp_loc, env, Apply_wrong_label (l', ty_fun')))
                else
                  ( [],
                    more_sargs,
    ...
    | _ -> (
        ...
        | (l, sarg0) :: _ when ignore_labels ->
            raise (Error (sarg0.pexp_loc, env, Apply_wrong_label (l, ty_old)))
        | _ -> type_unknown_args args omitted ty_fun0 (sargs @ more_sargs))
  in
  let is_ignore funct =
    match funct.exp_desc with
    | Texp_ident
        (_, _, { val_kind = Val_prim { Primitive.prim_name = "%ignore" } }) -> (
        try
  ...
  | _ ->
      ...
      if ignore_labels then type_args [] [] ty (instance env ty) ty [] sargs
      else type_args [] [] ty (instance env ty) ty sargs []
...
and type_statement env sexp =
  ...
  if !Clflags.strict_sequence then (
    ...
    (match ty.desc with
    | Tarrow _ -> Location.prerr_warning loc Warnings.Partial_application
    | Tconstr (p, _, _) when Path.same p Predef.path_unit -> ()
    | Tvar _ when ty.level > tv.level ->
        Location.prerr_warning loc Warnings.Nonreturning_statement
    | Tvar _ ->
        add_delayed_check (fun () -> check_application_result env true exp)
    | _ -> Location.prerr_warning loc Warnings.Statement_type);
    unify_var env tv ty;
    exp
...
and type_cases ?in_function env ty_arg ty_res partial_flag loc caselist =
  ...
  let ty_arg =
    if (has_gadts || erase_either) && not !Clflags.principal then
  ...
  let propagate =
    ...
    let rec is_var spat =
      ...
      | _ -> false
    in
    match caselist with [ { pc_lhs } ] when is_var pc_lhs -> false | _ -> true
  in
    ...
    List.map
      (fun { pc_lhs; pc_guard; pc_rhs } ->
        ...
        let pat, ext_env, force, unpacks =
          let partial =
            if !Clflags.principal || erase_either then Some false else None
          in
  ...
  in
  (if !Clflags.principal || has_gadts then
     let ty_res' = instance env ty_res in
     List.iter (fun c -> unify_exp env c.c_rhs ty_res') cases);
  let partial =
    if partial_flag then check_partial ~lev env ty_arg loc cases else Partial
  in
...
let report_error env ppf = function
  ...
  | Expr_type_clash trace ->
      report_unification_error ppf env trace
        ...
        (function ppf -> fprintf ppf "but an expression was expected of type")
  | Apply_non_function typ -> (
      ...
      | Tarrow _ ->
          ...
          fprintf ppf "@ @[It is applied to too many arguments;@ %s@]@]"
            "maybe you forgot a `;'."
      | _ ->
          fprintf ppf "@[<v>@[<2>This expression has type@ %a@]@ %s@]" type_expr
            typ "This is not a function; it cannot be applied.")
  ...
  | Label_missing labels ->
      ...
      in
      fprintf ppf "@[<hov>Some record fields are undefined:%a@]" print_labels
        labels
  ...
  | Wrong_name (eorp, ty, kind, p, name, valid_names) ->
      ...
      if Path.is_constructor_typath p then
        ...
        fprintf ppf "@[@[<2>%s type@ %a@]@ " eorp type_expr ty;
        fprintf ppf "The %s %s does not belong to type %a@]"
          (label_of_kind kind) name (*kind*) path p);
  ...
  | Virtual_class cl ->
      fprintf ppf "Cannot instantiate the virtual class %a" longident cl
  ...
  | Instance_variable_not_mutable (b, v) ->
      if b then fprintf ppf "The instance variable %s is not mutable" v
      else fprintf ppf "The value %s is not an instance variable" v
  | Not_subtype (tr1, tr2) ->
          ...
          | ppf ->
              ...
              fprintf ppf
                "This expression cannot be coerced to type@;\
                 <1 2>%a;@ it has type" (type_expansion ty) ty')
...
let () = Env.add_delayed_check_forward := add_delayed_check
