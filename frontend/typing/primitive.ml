...
let parse_declaration valdecl ~native_repr_args ~native_repr_res =
  ...
  if old_style_float then
    ...
    Location.prerr_warning valdecl.pval_loc
      (Warnings.Deprecated "[@@noalloc] should be used instead of \"noalloc\"");
  if
    native_name = ""
    && not
         (List.for_all is_ocaml_repr native_repr_args
...
let report_error ppf err =
  ...
  | No_native_primitive_with_repr_attribute ->
      Format.fprintf ppf
        "The native code version of the primitive is mandatory when attributes \
         [%@untagged] or [%@unboxed] are present"
