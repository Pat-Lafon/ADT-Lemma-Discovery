...
module EnvTbl = struct
  ...
  let find_same_not_using id tbl = fst (Ident.find_same id tbl)
  ...
  let find_all s tbl = Ident.find_all s tbl
...
let empty =
  {
    values = EnvTbl.empty;
    constrs = EnvTbl.empty;
    labels = EnvTbl.empty;
    types = EnvTbl.empty;
    modules = EnvTbl.empty;
    modtypes = EnvTbl.empty;
    components = EnvTbl.empty;
    classes = EnvTbl.empty;
...
let diff_keys is_local tbl1 tbl2 =
  ...
  List.filter
    (fun id ->
      ...
      &&
      try
        ignore (EnvTbl.find_same_not_using id tbl1);
        false
      with Not_found -> true)
    keys2
...
let components_of_module' =
  ref
    (fun ~deprecated env sub path mty -> assert false
      : deprecated:string option ->
        t ->
        Subst.t ->
        Path.t ->
...
let read_pers_struct check modname filename =
  ...
  let deprecated =
    List.fold_left
      (fun acc -> function Deprecated s -> Some s | _ -> acc)
      None flags
  in
  let comps =
...
let check_pers_struct name =
  if not (Hashtbl.mem persistent_structures name) then (
    ...
    if Warnings.is_active (Warnings.No_cmi_file ("", None)) then
      !add_delayed_check_forward (fun () -> check_pers_struct name))
...
let rec find_module_descr path env =
  ...
  | Pident id -> (
      try
        let p, desc = EnvTbl.find_same id env.components in
        desc
      with Not_found ->
...
let find_type_full path env =
  ...
  | LocalExt id ->
      let cstr =
        try EnvTbl.find_same id env.constrs with Not_found -> assert false
      in
      type_of_cstr path cstr
  | Ext (mod_path, s) -> (
      let comps =
      ...
      let exts =
        List.filter
          ...
          (try Tbl.find s comps.comp_constrs with Not_found -> assert false)
      in
      match exts with
...
let find_module ~alias path env =
  ...
  | Pident id -> (
      try
        let p, data = EnvTbl.find_same id env.modules in
        data
      with Not_found ->
        if Ident.persistent id && not (Ident.name id = !current_unit) then
...
let find_type_expansion path env =
  ...
  | _ ->
      ...
      if Path.same path path' then raise Not_found
      else
        ( decl.type_params,
...
let report_deprecated ?loc p deprecated =
  ...
  | Some loc, Some txt ->
      ...
      Location.prerr_warning loc
        (Warnings.Deprecated (Printf.sprintf "module %s%s" (Path.name p) txt))
  | _ -> ()

let rec lookup_module_descr_aux ?loc lid env =
  match lid with
  | Lident s -> (
      try EnvTbl.find_name s env.components
      with Not_found ->
...
let lookup proj1 proj2 ?loc lid env =
  match lid with
  | Lident s -> EnvTbl.find_name s (proj1 env)
  | Ldot (l, s) -> (
...
let lookup_all_simple proj1 proj2 shadow ?loc lid env =
  ...
  | Ldot (l, s) -> (
      ...
      match get_components desc with
      | Structure_comps c ->
          let comps = try Tbl.find s (proj2 c) with Not_found -> [] in
          List.map (fun (data, pos) -> (data, fun () -> ())) comps
      | Functor_comps f -> raise Not_found)
  | Lapply (l1, l2) -> raise Not_found
...
and lookup_type = lookup (fun env -> env.types) (fun sc -> sc.comp_types)

and lookup_modtype =
  lookup (fun env -> env.modtypes) (fun sc -> sc.comp_modtypes)

and lookup_class = lookup (fun env -> env.classes) (fun sc -> sc.comp_classes)
and lookup_cltype = lookup (fun env -> env.cltypes) (fun sc -> sc.comp_cltypes)
...
let update_value s f env =
  try
    ...
    | Pident id ->
        ...
        {
          env with
          values = Ident.add id ((p, vd2), slot) env.values;
...
let is_lident = function Lident _ -> true | _ -> false
...
let rec find_shadowed_comps path env =
  match path with
  | Pident id -> List.map fst (Ident.find_all (Ident.name id) env.components)
  | Pdot (p, s, _) ->
...
let find_shadowed proj1 proj2 path env =
  match path with
  | Pident id -> List.map fst (Ident.find_all (Ident.name id) (proj1 env))
  | Pdot (p, s, _) ->
      let l = find_shadowed_comps p env in
...
let rec scrape_alias env ?path mty =
  ...
  | Mty_alias path, _ -> (
      ...
      with Not_found ->
        ...
        mty)
  | mty, Some path -> !strengthen env mty path
  | _ -> mty
...
let rec prefix_idents root pos sub = function
  ...
  | Sig_value (id, decl) :: rem ->
      ...
      let nextpos =
        match decl.val_kind with Val_prim _ -> pos | _ -> pos + 1
      in
...
let subst_signature sub sg =
  List.map
    (fun item ->
      match item with
      | Sig_value (id, decl) -> Sig_value (id, Subst.value_description sub decl)
      | Sig_type (id, decl, x) ->
...
let prefix_idents_and_subst root sub sg =
  ...
  if sub = set_nongen_level Subst.identity root then (
    ...
    with Not_found ->
      ...
      sgs := (sg, r) :: !sgs;
      r)
  else prefix_idents_and_subst root sub sg
...
let rec components_of_module ~deprecated env sub path mty =
  { deprecated; comps = EnvLazy.create (env, sub, path, mty) }
      ...
      List.iter2
        (fun item path ->
          ...
          | Sig_value (id, decl) -> (
              ...
              match decl.val_kind with Val_prim _ -> () | _ -> incr pos)
          | Sig_type (id, decl, _) ->
...
and store_value ?check slot id path decl env renv =
  ...
  may (fun f -> check_usage decl.val_loc id f value_declarations) check;
  {
    env with
    values =
      EnvTbl.add slot (fun x -> `Value x) id (path, decl) env.values renv.values;
   ...
   then
     ...
     if not (Hashtbl.mem used_constructors k) then (
       ...
       !add_delayed_check_forward (fun () ->
           if (not (is_in_signature env)) && not used.cu_positive then
             Location.prerr_warning loc
               (Warnings.Unused_extension (n, used.cu_pattern, used.cu_privatize)))));
  {
    env with
    constrs =
      EnvTbl.add slot
...
and store_module slot id path md env renv =
  let deprecated = Builtin_attributes.deprecated_of_attrs md.md_attributes in
  {
    ...
    modules =
      EnvTbl.add slot
        (fun x -> `Module x)
        id (path, md) env.modules renv.modules;
    components =
      EnvTbl.add slot
        (fun x -> `Component x)
        id
        ( path,
          components_of_module ~deprecated env Subst.identity path md.md_type )
...
and store_modtype slot id path info env renv =
  {
    ...
    modtypes =
      EnvTbl.add slot
        (fun x -> `Module_type x)
        id (path, info) env.modtypes renv.modtypes;
...
and store_class slot id path desc env renv =
  {
    ...
    classes =
      EnvTbl.add slot
        (fun x -> `Class x)
        id (path, desc) env.classes renv.classes;
...
and store_cltype slot id path desc env renv =
  {
    ...
    cltypes =
      EnvTbl.add slot
        (fun x -> `Class_type x)
        id (path, desc) env.cltypes renv.cltypes;
...
let components_of_functor_appl f env p1 p2 =
  ...
  with Not_found ->
    ...
    let comps =
      components_of_module ~deprecated:None (*???*) env Subst.identity p mty
    in
...
and add_modtype id info env = store_modtype None id (Pident id) info env env
and add_class id ty env = store_class None id (Pident id) ty env env
and add_cltype id ty env = store_cltype None id (Pident id) ty env env

let add_module ?arg id mty env = add_module_declaration ?arg id (md mty) env
...
let add_local_constraint id info elv env =
  ...
  | { type_manifest = Some ty; type_newtype_level = Some (lv, _) } ->
      ...
      let env =
        add_type ~check:false id
          { info with type_newtype_level = Some (lv, elv) }
          env
      in
...
let enter store_fun name data env =
  ...
  (id, store_fun None id (Pident id) data env env)
...
let find_all_simple_list proj1 proj2 f lid env acc =
  match lid with
  | None -> EnvTbl.fold_name (fun id data acc -> f data acc) (proj1 env) acc
  | Some l -> (
      let p, desc = lookup_module_descr l env in
      match get_components desc with
      | Structure_comps c ->
          Tbl.fold
            (fun s comps acc ->
              match comps with [] -> acc | (data, pos) :: _ -> f data acc)
            (proj2 c) acc
      | Functor_comps _ -> acc)
