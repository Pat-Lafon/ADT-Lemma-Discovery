...
module MakeIterator (Iter : IteratorArgument) : sig
  ...
  let may_iter f v = match v with None -> () | Some x -> f x
  ...
  and iter_structure_item item =
    ...
    | Tstr_class_type list ->
        List.iter (fun (id, _, ct) -> iter_class_type_declaration ct) list
  ...
  and iter_type_parameter (ct, v) = iter_core_type ct
  ...
  and iter_pattern pat =
    ...
    | Tpat_variant (label, pato, _) -> (
        match pato with None -> () | Some pat -> iter_pattern pat)
  ...
  and iter_expression exp =
    ...
    | Texp_apply (exp, list) ->
        ...
        List.iter
          (fun (label, expo) ->
            match expo with None -> () | Some exp -> iter_expression exp)
          list
    ...
    | Texp_record (list, expo) -> (
        ...
        match expo with None -> () | Some exp -> iter_expression exp)
    | Texp_field (exp, _, label) -> iter_expression exp
    ...
    | Texp_setinstvar (_, _, _, exp) -> iter_expression exp
    | Texp_override (_, list) ->
        List.iter (fun (path, _, exp) -> iter_expression exp) list
    | Texp_letmodule (id, _, mexpr, exp) ->
        iter_module_expr mexpr;
        iter_expression exp
    | Texp_assert exp -> iter_expression exp
    | Texp_lazy exp -> iter_expression exp
    | Texp_object (cl, _) -> iter_class_structure cl
  ...
  and iter_signature_item item =
    ...
    | Tsig_class list -> List.iter iter_class_description list
  ...
  and iter_module_type mty =
    ...
    | Tmty_with (mtype, list) ->
        ...
        List.iter (fun (path, _, withc) -> iter_with_constraint withc) list
  ...
  and iter_module_expr mexpr =
    ...
    | Tmod_constraint (mexpr, _, Tmodtype_explicit mtype, _) ->
        iter_module_expr mexpr;
        iter_module_type mtype
    | Tmod_unpack (exp, mty) -> iter_expression exp)
  ...
  and iter_class_expr cexpr =
    ...
    | Tcl_constraint (cl, None, _, _, _) -> iter_class_expr cl
    ...
    | Tcl_ident (_, _, tyl) -> List.iter iter_core_type tyl);
  ...
  and iter_class_type ct =
    ...
    | Tcty_constr (path, _, list) -> List.iter iter_core_type list
  ...
  and iter_class_type_field ctf =
    ...
    | Tctf_val (s, _mut, _virt, ct) -> iter_core_type ct
    | Tctf_method (s, _priv, _virt, ct) -> iter_core_type ct
  ...
  and iter_core_type ct =
    ...
    | Ttyp_constr (path, _, list) -> List.iter iter_core_type list
    | Ttyp_object (list, o) ->
        List.iter (fun (_, _, t) -> iter_core_type t) list
    | Ttyp_class (path, _, list) -> List.iter iter_core_type list
    | Ttyp_alias (ct, s) -> iter_core_type ct
    | Ttyp_variant (list, bool, labels) -> List.iter iter_row_field list
  ...
  and iter_row_field rf =
    match rf with
    | Ttag (label, _attrs, bool, list) -> List.iter iter_core_type list
    | Tinherit ct -> iter_core_type ct

  and iter_class_field cf =
    Iter.enter_class_field cf;
    (match cf.cf_desc with
    | Tcf_inherit (ovf, cl, super, _vals, _meths) -> iter_class_expr cl
    | Tcf_constraint (cty, cty') ->
        iter_core_type cty;
        iter_core_type cty'
    | Tcf_val (lab, _, _, Tcfk_virtual cty, _) -> iter_core_type cty
    | Tcf_val (lab, _, _, Tcfk_concrete (_, exp), _) -> iter_expression exp
    | Tcf_method (lab, _, Tcfk_virtual cty) -> iter_core_type cty
    | Tcf_method (lab, _, Tcfk_concrete (_, exp)) -> iter_expression exp
    | Tcf_initializer exp -> iter_expression exp
    | Tcf_attribute _ -> ());
