...
let rec constructor_type constr cty =
  ...
  | Cty_signature sign -> constr
  | Cty_arrow (l, ty, cty) ->
      Ctype.newty (Tarrow (l, ty, constructor_type constr cty, Cok))

let rec class_body cty =
  match cty with
  | Cty_constr (_, _, cty') -> cty (* Only class bodies can be abbreviated *)
  | Cty_signature sign -> cty
  | Cty_arrow (_, ty, cty) -> class_body cty
...
let enter_val cl_num vars inh lab mut virt ty val_env met_env par_env loc =
  ...
  let ((id, _, _, _) as result) =
    match id with
    | Some id -> (id, val_env, met_env, par_env)
    | None ->
...
let rec class_type_field env self_type meths
  ...
  | Pctf_inherit sparent ->
      ...
      in
      let val_sig =
        Vars.fold (add_val env sparent.pcty_loc) cl_sig.csig_vars val_sig
      in
  ...
  | Pctf_val (lab, mut, virt, sty) ->
      ...
      ( mkctf (Tctf_val (lab, mut, virt, cty)) :: fields,
        add_val env ctf.pctf_loc lab (mut, virt, ty) val_sig,
        concr_meths,
        inher )
...
let rec class_field self_loc cl_num self_type meths vars
  ...
  | Pcf_inherit (ovf, sparent, super) ->
      ...
      let inh_meths =
        Concr.fold
          (fun lab rem -> (lab, Ident.create lab) :: rem)
          cl_sig.csig_concr []
      in
      (* Super *)
      let val_env, met_env, par_env =
        match super with
        | None -> (val_env, met_env, par_env)
        | Some name ->
            let id, val_env, met_env, par_env =
              enter_met_env
  ...
  | Pcf_val (lab, mut, Cfk_virtual styp) ->
      ...
      let id, val_env, met_env', par_env =
        enter_val cl_num vars false lab.txt mut Virtual ty val_env met_env
          par_env loc
      in
      ( val_env,
        met_env',
        par_env,
        lazy
          (mkcf (Tcf_val (lab, mut, id, Tcfk_virtual cty, met_env == met_env')))
        :: fields,
        concr_meths,
        warn_vals,
        inher,
  ...
  | Pcf_val (lab, mut, Cfk_concrete (ovf, sexp)) ->
      ...
      let id, val_env, met_env', par_env =
        enter_val cl_num vars false lab.txt mut Concrete exp.exp_type val_env
          met_env par_env loc
      in
      ( val_env,
        met_env',
        ...
        lazy
          (mkcf
             (Tcf_val
                (lab, mut, id, Tcfk_concrete (ovf, exp), met_env == met_env')))
        :: fields,
        concr_meths,
        ...
        lazy
          (Ctype.raise_nongen_level ();
           let meth_type =
             Ctype.newty
               (Tarrow
                  (Nolabel, self_type, Ctype.instance_def Predef.type_unit, Cok))
           in
           vars := vars_local;
      ...
      List.fold_right
        (fun (lab, kind, ty) rem ->
          if lab = dummy_method then
            ...
            | Fvar r ->
                Btype.set_kind r Fabsent;
                rem
            | _ -> rem
          else Ctype.newty (Tfield (lab, Btype.copy_kind kind, ty, rem)))
...
and class_expr cl_num val_env met_env scl =
  ...
  | Pcl_fun (l, None, spat, scl') ->
      ...
      if !Clflags.principal then (
        Ctype.end_def ();
        iter_pattern
          (fun { pat_type = ty } -> Ctype.generalize_structure ty)
          pat);
      let pv =
        List.map
          (fun (id, id_loc, id', ty) ->
          ...
          pv
      in
      let not_function = function Cty_arrow _ -> false | _ -> true in
      let partial =
      ...
      rc
        {
          ...
          cl_loc = scl.pcl_loc;
          cl_type = Cty_arrow (l, Ctype.instance_def pat.pat_type, cl.cl_type);
          cl_env = val_env;
          cl_attributes = scl.pcl_attributes;
        }
  | Pcl_apply (scl', sargs) ->
        ...
        | Cty_arrow (l, ty, ty_fun), Cty_arrow (_, ty0, ty_fun0)
          when sargs <> [] || more_sargs <> [] ->
            ...
            let sargs, more_sargs, arg =
              if ignore_labels && not (Btype.is_optional l) then
                match (sargs, more_sargs) with
                | (l', sarg0) :: _, _ ->
                    raise
                      (Error (sarg0.pexp_loc, val_env, Apply_wrong_label l'))
                | _, (l', sarg0) :: more_sargs ->
                    if l <> l' && l' <> Nolabel then
                      raise
                        (Error (sarg0.pexp_loc, val_env, Apply_wrong_label l'))
                    else
                      ([], more_sargs, Some (type_argument val_env sarg0 ty ty0))
                | _ -> assert false
              else
                try
                ...
                with Not_found ->
                  ( sargs,
                    ...
                    then Some (option_none ty0 Location.none)
                    else None )
            in
      ...
      let args, cty =
        ...
        if ignore_labels then type_args [] [] cl.cl_type ty_fun0 [] sargs
        else type_args [] [] cl.cl_type ty_fun0 sargs []
      in
      rc
        {
  ...
  | Pcl_let (rec_flag, sdefs, scl') ->
      let defs, val_env =
        try Typecore.type_let val_env rec_flag sdefs None
        with Ctype.Unify [ (ty, _) ] ->
      ...
      let vals, met_env =
        List.fold_right
          (fun (id, id_loc) (vals, met_env) ->
            ...
            ((id', id_loc, expr) :: vals, Env.add_value id' desc met_env))
...
let initial_env define_class approx (res, env) (cl, id, ty_id, obj_id, cl_id) =
...
let class_infos define_class kind
  ...
  (if cl.pci_virt = Concrete then
     ...
     if mets <> [] || vals <> [] then
       raise
         (Error
            (cl.pci_loc, env, Virtual_class (define_class, false, mets, vals))));
...
let type_object env loc s =
  incr class_num;
  let desc, sign =
    class_structure (string_of_int !class_num) true env env loc s
  in
...
let report_error env ppf = function
  ...
  | Bad_parameters (id, params, cstrs) ->
      ...
      fprintf ppf
        "@[The abbreviation %a@ is used with parameters@ %a@ wich are \
  ...
  | Make_nongen_seltype ty ->
      fprintf ppf
        "@[<v>@[Self type should not occur in the non-generic type@;\
         <1 2>%a@]@,\
         It would escape the scope of its class@]" Printtyp.type_scheme ty
