...
let rec strengthen env mty p =
  ...
  | Mty_signature sg -> Mty_signature (strengthen_sig env sg p 0)
...
and strengthen_sig env sg p pos =
  ...
  | (Sig_value (id, desc) as sigelt) :: rem ->
      let nextpos =
        match desc.val_kind with Val_prim _ -> pos | _ -> pos + 1
      in
  ...
  | Sig_module (id, md, rs) :: rem ->
      let str =
        ...
        else { md with md_type = Mty_alias (Pdot (p, Ident.name id, pos)) }
      in
  ...
  | Sig_modtype (id, decl) :: rem ->
      let newdecl =
        ...
        | None ->
            {
              decl with
              mtd_type = Some (Mty_ident (Pdot (p, Ident.name id, nopos)));
            }
        | Some _ -> decl
      in
...
and strengthen_decl env md p = { md with md_type = strengthen env md.md_type p }
    ...
    | item :: rem -> (
        ...
        | Sig_modtype (id, d) -> (
            ...
            with Not_found -> (
              ...
              | _ -> raise Not_found))
        | Sig_class (id, d, rs) ->
            Sig_class (id, Ctype.nondep_class_declaration env mid d, rs) :: rem'
...
and enrich_item env p = function
  | Sig_type (id, decl, rs) ->
      Sig_type
        (id, enrich_typedecl env (Pdot (p, Ident.name id, nopos)) decl, rs)
  | Sig_module (id, md, rs) ->
      Sig_module
        ( id,
          {
...
and no_code_needed_sig env sg =
  ...
  | (Sig_typext _ | Sig_class _) :: rem -> false
...
module P = struct
  ...
  let compare p1 p2 = if Path.same p1 p2 then 0 else compare p1 p2
end
...
let rec remove_aliases env excl mty =
  ...
  | Mty_alias _ ->
      ...
      if mty' = mty then mty else remove_aliases env excl mty'
  | mty -> mty
