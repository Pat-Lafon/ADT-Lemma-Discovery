...
let set_object_name id rv params ty =
  ...
  | Tobject (fi, nm) -> set_name nm (Some (Path.Pident id, rv :: params))
  | _ -> assert false
  ...
  try
    ...
    | Type_variant v ->
        List.iter
          ...
          v
    | Type_record (r, rep) -> List.iter (fun l -> closed_type l.ld_type) r
    | Type_open -> ());
  ...
  if ty.level <> generic_level then
    if ty.level > var_level then (
      ...
      | Tconstr (path, tyl, abbrev) ->
          ...
          List.iter2
            (fun v t ->
              if Variance.(mem May_weak v) then
                generalize_contravariant env var_level t
      ...
      | _ -> iter_type_expr (generalize_expansive env var_level) ty)

let generalize_expansive env ty =
  simple_abbrevs := Mnil;
  try generalize_expansive env !nongen_level ty
...
let expand_abbrev_gen kind find_type_expansion env ty =
  ...
  | { desc = Tconstr (path, args, abbrev); level } -> (
      ...
      | None ->
          let params, body, lv =
            try find_type_expansion path env
            with Not_found -> raise Cannot_expand
          in
          (* prerr_endline
...
let rec local_non_recursive_abbrev visited env p ty =
  ...
  if not (List.memq ty visited) then
    ...
    | Tconstr (p', args, abbrev) -> (
        ...
        if is_contractive env p' then ()
        else
          let visited = ty :: visited in
          try List.iter (local_non_recursive_abbrev visited env p) args
          with Occur -> (
            try
...
let local_non_recursive_abbrev env p ty =
  try
    local_non_recursive_abbrev [] env p ty;
    true
  with Occur -> false
...
and mcomp_fields type_pairs env ty1 ty2 =
  ...
  if
    (miss1 <> [] && (object_row ty1).desc = Tnil)
    || (miss2 <> [] && (object_row ty2).desc = Tnil)
  then raise (Unify []);
  List.iter
    (function
      | n, k1, t1, k2, t2 ->
...
and unify2 env t1 t2 =
  ...
  if unify_eq !env t1' t2' then ()
    ...
    (if !trace_gadt_instances then
       ...
       let ilevel t =
         match Env.gadt_instance_level !env t with None -> 0 | Some lv -> lv
       in
...
and unify_row env row1 row2 =
  ...
  if unify_eq !env rm1 rm2 then ()
    ...
    let more =
      ...
      else newty2 (min rm1.level rm2.level) (Tvar None)
    in
    ...
    let set_more row rest =
      ...
      if !trace_gadt_instances && rm.desc = Tnil then ()
      else if !trace_gadt_instances then
      ...
      List.iter
        (fun (l, f1, f2) ->
          ...
          with Unify trace ->
            raise
              (Unify
                 ((mkvariant [ (l, f1) ] true, mkvariant [ (l, f2) ] true)
                 :: trace)))
        pairs
    with exn ->
      log_type rm1;
  ...
  let error =
    List.fold_right
      (fun (lab, k, _) err ->
        let err =
          let k = field_kind_repr k in
          match k with Fvar r -> err | _ -> CM_Hide_public lab :: err
        in
        if Concr.mem lab sign1.csig_concr then err
        else CM_Hide_virtual ("method", lab) :: err)
      miss1 []
...
let rec lid_of_path ?(sharp = "") = function
  | Path.Pident id -> Longident.Lident (sharp ^ Ident.name id)
  | Path.Pdot (p1, s, _) -> Longident.Ldot (lid_of_path p1, sharp ^ s)
  | Path.Papply (p1, p2) ->
...
let rec arity ty =
  match (repr ty).desc with Tarrow (_, t1, t2, _) -> 1 + arity t2 | _ -> 0
        ...
        ty'.desc <-
          ...
          | Tvariant row -> (
              ...
              with Not_found -> (
                ...
                (* Return a new copy *)
                let row =
                  copy_row (nondep_type_rec env id) true row true more'
                in
                match row.row_name with
