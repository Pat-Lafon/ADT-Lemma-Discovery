...
let ensure_functor_arg p env =
  if Env.is_functor_arg p env then env
  else Env.add_functor_arg (Path.head p) env
  ...
  let rec merge env sg namelist row_id =
    ...
    | Sig_module (id, md, rs) :: rem, [ s ], Pwith_module (_, lid')
      when Ident.name id = s ->
        ...
        let md'' =
          { md' with md_type = Mtype.remove_aliases env md'.md_type }
        in
...
let rec approx_modtype env smty =
  ...
  | Pmty_with (sbody, constraints) -> approx_modtype env sbody
  | Pmty_typeof smod ->
...
let anchor_recmodule id anchor = Some (Pident id)
...
let check_recmodule_inclusion env bindings =
  ...
  let rec check_incl first_time n env s =
    if n > 0 then
      ...
      let s' =
        List.fold_left
          (fun s (id, id', mty_actual) -> Subst.add_module id (Pident id') s)
      ...
      let check_inclusion (id, id_loc, mty_decl, modl, mty_actual, attrs, loc) =
        ...
        let coercion =
          try Includemod.modtypes env mty_actual' mty_decl'
...
let package_subtype env p1 nl1 tl1 p2 nl2 tl2 =
  let mkmty p nl tl =
    let ntl =
      List.filter
        (fun (n, t) -> Ctype.free_variables t = [])
        (List.combine nl tl)
    in
...
let wrap_constraint env arg mty explicit =
  let coercion =
    try Includemod.modtypes env arg.mod_type mty
    with Includemod.Error msg ->
...
let rec type_module ?(alias = false) sttn funct_body anchor env smod =
  ...
  | Pmod_ident lid ->
      ...
      let md =
        if alias && not (Env.is_functor_arg path env) then (
          ...
          | mty ->
              let mty = if sttn then Mtype.strengthen env mty path else mty in
  ...
  | Pmod_apply (sfunct, sarg) -> (
      ...
      | Mty_functor (param, mty_param, mty_res) as mty_functor ->
          ...
          let coercion =
            try Includemod.modtypes env arg.mod_type mty_param
            with Includemod.Error msg ->
...
and type_structure ?(toplevel = false) funct_body anchor env sstr scope =
  ...
  let type_str_item env srem { pstr_loc = loc; pstr_desc = desc } =
    ...
    | Pstr_eval (sexpr, attrs) ->
        let expr =
          Builtin_attributes.with_warning_attribute attrs (fun () ->
    ...
    | Pstr_value (rec_flag, sdefs) ->
        let scope =
          ...
          | Recursive ->
              Some
                (Annot.Idef
                   { scope with Location.loc_start = loc.Location.loc_start })
          | Nonrecursive ->
              let start =
                match srem with
    ...
    | Pstr_class cl ->
        ...
        ( Tstr_class
            (List.map
               (fun (_, _, _, _, _, _, _, _, _, _, m, c) -> (c, m))
               classes),
    ...
    | Pstr_class_type cl ->
        ...
        ( Tstr_class_type
            (List.map
               (fun (i, i_loc, d, _, _, _, _, c) -> (i, i_loc, c))
               classes),
          (* TODO: check with Jacques why this is here
                    Tstr_type
...
let type_package env m p nl tl =
  ...
  let mp, env =
    ...
    | Tmod_constraint
        ({ mod_desc = Tmod_ident (mp, _) }, mty, Tmodtype_implicit, _) ->
        (mp, env) (* PR#6982 *)
    | _ ->
    ...
    if !Clflags.print_types then (
      ...
      if Sys.file_exists sourceintf then (
        ...
        (if not !Clflags.dont_write_files then
           ...
           let sg =
             Env.save_signature ~deprecated simple_sg modulename
               (outputprefix ^ ".cmi")
           in
           Cmt_format.save_cmt (outputprefix ^ ".cmt") modulename
...
let rec package_signatures subst = function
  ...
  | (name, sg) :: rem ->
      ...
      let oldid = Ident.create_persistent name and newid = Ident.create name in
      Sig_module
        ( newid,
          {
...
let package_units initial_env objfiles cmifile modulename =
  ...
  if Sys.file_exists mlifile then (
    ...
    (if not !Clflags.dont_write_files then
       let sg =
         Env.save_signature_with_imports ~deprecated:None sg modulename
           (prefix ^ ".cmi") imports
       in
       Cmt_format.save_cmt (prefix ^ ".cmt") modulename
         (Cmt_format.Packed (sg, objfiles))
         None initial_env (Some sg));
    Tcoerce_none
