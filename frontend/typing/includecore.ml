  ...
  | ( { Types.cd_id = cstr1; cd_args = arg1; cd_res = ret1 } :: rem1,
      ...
      if Ident.name cstr1 <> Ident.name cstr2 then
        ...
        let r =
          ...
          | Some _, None | None, Some _ -> [ Field_type cstr1 ]
          | _ ->
              compare_constructor_arguments env cstr1 params1 params2 arg1 arg2
        in
        if r <> [] then r
...
and compare_records env params1 params2 n labels1 labels2 =
  ...
  | ( { Types.ld_id = lab1; ld_mutable = mut1; ld_type = arg1 } :: rem1,
      ...
      if Ident.name lab1 <> Ident.name lab2 then [ Field_names (n, lab1, lab2) ]
      else if mut1 <> mut2 then [ Field_mutable lab1 ]
      else if Ctype.equal env true (arg1 :: params1) (arg2 :: params2) then
        compare_records env params1 params2 (n + 1) rem1 rem2
    ...
    if err <> [] then err
      ...
      let err =
        ...
        | Type_variant cstrs1, Type_variant cstrs2 ->
            ...
            compare_variants env decl1.type_params decl2.type_params 1 cstrs1
              cstrs2
        | Type_record (labels1, rep1), Type_record (labels2, rep2) ->
            let err =
              compare_records env decl1.type_params decl2.type_params 1 labels1
                labels2
            in
            if err <> [] || rep1 = rep2 then err
            else [ Record_representation (rep2 = Record_float) ]
        | Type_open, Type_open -> []
        | _, _ -> [ Kind ]
      in
      if err <> [] then err
...
let extension_constructors env id ext1 ext2 =
  ...
  then
    if
      compare_constructor_arguments env (Ident.create "") ext1.ext_type_params
        ext2.ext_type_params ext1.ext_args ext2.ext_args
      = []
    then
      if
