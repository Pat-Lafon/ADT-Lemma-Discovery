...
let expression sub exp =
  ...
  let desc =
    ...
    | Texp_function (Nolabel, cases, _) -> Pexp_function (sub.cases sub cases)
...
let module_type sub mty =
  ...
  let desc =
    ...
    | Tmty_functor (_id, name, mtype1, mtype2) ->
        Pmty_functor
          ( name,
            map_opt (sub.module_type sub) mtype1,
...
let class_type_field sub ctf =
  ...
  let desc =
    ...
    | Tctf_val (s, mut, virt, ct) -> Pctf_val (s, mut, virt, sub.typ sub ct)
...
let core_type sub ct =
  ...
  let desc =
    ...
    | Ttyp_object (list, o) ->
        Ptyp_object (List.map (fun (s, a, t) -> (s, a, sub.typ sub t)) list, o)
