...
let value_descriptions env cxt subst id vd1 vd2 =
  ...
  try Includecore.value_descriptions env vd1 vd2
...
let extension_constructors env cxt subst id ext1 ext2 =
  ...
  if Includecore.extension_constructors env id ext1 ext2 then ()
...
let class_declarations ~old_env env cxt subst id decl1 decl2 =
  ...
  | reason ->
      raise
        (Error [ (cxt, old_env, Class_declarations (id, decl1, decl2, reason)) ])
...
let rec modtypes env cxt subst mty1 mty2 =
  try try_modtypes env cxt subst mty1 mty2 with
  | Dont_match ->
      raise
...
and try_modtypes env cxt subst mty1 mty2 =
  ...
  | _, Mty_ident p2 -> try_modtypes2 env cxt mty1 (Subst.modtype subst mty2)
  | Mty_signature sig1, Mty_signature sig2 -> signatures env cxt subst sig1 sig2
  | Mty_functor (param1, None, res1), Mty_functor (param2, None, res2) -> (
      match modtypes env (Body param1 :: cxt) subst res1 res2 with
...
and try_modtypes2 env cxt mty1 mty2 =
  ...
  match (mty1, mty2) with
  | Mty_ident p1, Mty_ident p2 when Path.same p1 p2 -> Tcoerce_none
  | _, Mty_ident p2 ->
...
and signature_components old_env env cxt subst paired =
  ...
  | (Sig_type (id1, tydecl1, _), Sig_type (id2, tydecl2, _), pos) :: rem ->
      ...
      comps_rec rem
  | (Sig_typext (id1, ext1, _), Sig_typext (id2, ext2, _), pos) :: rem ->
...
and modtype_infos env cxt subst id info1 info2 =
  ...
  try
    ...
    | None, None -> ()
    | Some mty1, None -> ()
    | Some mty1, Some mty2 -> check_modtype_equiv env cxt' mty1 mty2
    | None, Some mty2 ->
...
let check_modtype_inclusion env mty1 path1 mty2 =
  try
    ignore
      (modtypes env [] Subst.identity (Mtype.strengthen env mty1 path1) mty2)
  with Error reasons -> raise Not_found
...
let compunit env impl_name impl_sig intf_name intf_sig =
  try signatures env [] Subst.identity impl_sig intf_sig
  with Error reasons ->
    raise
      (Error
