...
let make_constructor loc env type_path type_params sargs sret_type =
  ...
  | None ->
      let args, targs = transl_constructor_arguments loc env true sargs in
  ...
  | Some sret_type ->
      ...
      let args, targs = transl_constructor_arguments loc env false sargs in
...
let transl_declaration env sdecl id =
  ...
  let tkind, kind =
    ...
    | Ptype_record lbls ->
        ...
        let rep =
          if List.for_all (fun l -> is_float env l.Types.ld_type) lbls' then
            Record_float
          else Record_regular
        in
...
let check_well_founded env loc path to_check ty =
  ...
  let rec check ty0 exp_nodes ty =
    ...
    if TypeSet.mem ty exp_nodes then
    ...
    let fini, exp_nodes =
      try
        ...
        if TypeSet.subset exp_nodes prev then (true, exp_nodes)
        else (false, TypeSet.union exp_nodes prev)
      with Not_found -> (false, exp_nodes)
    in
    ...
    if fini then ()
      ...
      | Ctype.Cannot_expand ->
          ...
          in
          let nodes = if rec_ok then TypeSet.empty else exp_nodes in
...
let rec compute_properties_fixpoint env decls required variances immediacies =
  ...
  in
  let new_immediacies =
    List.map (fun (id, decl) -> compute_immediacy new_env decl) new_decls
  in
...
let transl_type_decl env rec_flag sdecl_list =
  ...
  *)
  ...
  let final_decls =
    List.map2
      (fun tdecl (id2, decl) -> { tdecl with typ_type = decl })
      tdecls final_decls
  in
  (* Done *)
...
let transl_extension_constructor env type_path type_params typext_params priv
  ...
  let args, ret_type, kind =
    ...
    | Pext_decl (sargs, sret_type) ->
        let targs, tret_type, args, ret_type =
          make_constructor sext.pext_loc env type_path typext_params sargs
            sret_type
        in
...
let report_error ppf = function
  ...
  | Not_open_type path ->
      fprintf ppf "@[%s@ %a@]" "Cannot extend type definition" Printtyp.path
        path
  | Not_extensible_type path ->
      fprintf ppf "@[%s@ %a@ %s@]" "Type" Printtyp.path path "is not extensible"
