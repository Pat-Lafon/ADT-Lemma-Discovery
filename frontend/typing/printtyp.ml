...
let rec tree_of_path = function
  ...
  | Pdot (Pident id, s, pos) when Ident.same id ident_pervasive -> Oide_ident s
  | Pdot (p, s, pos) -> Oide_dot (tree_of_path p, s)
...
let set_printing_env env =
  ...
  if !printing_env == Env.empty || same_printing_env env then ()
  else (
...
let rec tree_of_typexp sch ty =
  ...
  if List.mem_assq px !names && not (List.memq px !delayed) then
    ...
    let pr_typ () =
      match ty.desc with
      | Tvar _ -> Otyp_var (is_non_gen sch ty, name_of_type ty)
      | Tarrow (l, ty1, ty2, _) ->
      ...
      | Tconstr (p, tyl, abbrev) ->
          ...
          if is_nth s then tree_of_typexp sch (List.hd tyl')
      ...
      | Tpoly (ty, tyl) ->
          ...
          if tyl = [] then tree_of_typexp sch ty
            ...
            delayed := old_delayed;
            tr
      | Tunivar _ -> Otyp_var (false, name_of_type ty)
      | Tpackage (p, n, tyl) ->
          let n =
...
let typexp sch prio ppf ty = !Oprint.out_type ppf (tree_of_typexp sch ty)
...
let rec tree_of_type_decl id decl =
  ...
  | Type_variant cstrs ->
      List.iter
        ...
        cstrs
  | Type_record (l, rep) -> List.iter (fun l -> mark_loops l.ld_type) l
  | Type_open -> ());
  ...
  let ty, priv =
    ...
    | Type_record (lbls, rep) ->
        ( tree_of_manifest (Otyp_record (List.map tree_of_label lbls)),
          decl.type_private )
    | Type_open -> (tree_of_manifest Otyp_open, Public)
  in
  let immediate =
...
let rec tree_of_class_type sch params = function
  ...
  | Cty_signature sign ->
      let sty = repr sign.csig_self in
      let self_ty =
        if is_aliased sty then Some (Otyp_var (false, name_of_type (proxy sty)))
        else None
      in
      let fields, _ =
        Ctype.flatten_fields (Ctype.object_fields sign.csig_self)
...
let rec tree_of_modtype ?(ellipsis = false) = function
  ...
  | Mty_alias p -> Omty_alias (tree_of_path p)
...
and trees_of_sigitem = function
  ...
  | Sig_module (id, md, rs) ->
      let ellipsis =
        List.exists
          (function { txt = "..." }, Parsetree.PStr [] -> true | _ -> false)
          md.md_attributes
      in
      [ tree_of_module id md.md_type rs ~ellipsis ]
  | Sig_modtype (id, decl) -> [ tree_of_modtype_declaration id decl ]
  | Sig_class (id, decl, rs) -> [ tree_of_class_declaration id decl rs ]
...
let rec filter_trace keep_last = function
  ...
  | (t1, t1') :: (t2, t2') :: rem ->
      ...
      if
        is_constr_row t1' || is_constr_row t2'
...
let rec mismatch unif = function
  | (_, t) :: (_, t') :: rem -> (
      match mismatch unif rem with
      | Some _ as m -> m
      | None -> if has_explanation unif t t' then Some (t, t') else None)
  | [] -> None
  | _ -> assert false
...
let explanation unif t3 t4 ppf =
  ...
  | Tconstr (p, tl, _), Tvar _ when unif && t4.level < Path.binding_time p ->
      fprintf ppf "@,@[The type constructor@;<1 2>%a@ would escape its scope@]"
        path p
  | Tvar _, Tconstr (p, tl, _) when unif && t3.level < Path.binding_time p ->
      fprintf ppf "@,@[The type constructor@;<1 2>%a@ would escape its scope@]"
        path p
  ...
  | (Tnil | Tconstr _), Tfield (l, _, _, _) ->
      fprintf ppf "@,@[The first object type has no method %s@]" l
  | Tfield (l, _, _, _), (Tnil | Tconstr _) ->
      fprintf ppf "@,@[The second object type has no method %s@]" l
...
let explanation unif mis ppf =
  match mis with None -> () | Some (t3, t4) -> explanation unif t3 t4 ppf
