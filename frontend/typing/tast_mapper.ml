...
let expr sub x =
  ...
  let exp_desc =
    ...
    | Texp_function (l, cases, p) -> Texp_function (l, sub.cases sub cases, p)
    | Texp_apply (exp, list) ->
        Texp_apply
    ...
    | Texp_record (list, expo) ->
        Texp_record
          (List.map (tuple3 id id (sub.expr sub)) list, opt (sub.expr sub) expo)
...
let class_expr sub x =
  ...
  let cl_desc =
    ...
    | Tcl_apply (cl, args) ->
        Tcl_apply
          (sub.class_expr sub cl, List.map (tuple2 id (opt (sub.expr sub))) args)
...
let typ sub x =
  ...
  let ctyp_desc =
    ...
    | Ttyp_object (list, closed) ->
        Ttyp_object (List.map (tuple3 id id (sub.typ sub)) list, closed)
