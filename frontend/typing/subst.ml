...
let rec module_path s = function
  | Pident id as p -> ( try Tbl.find id s.modules with Not_found -> p)
...
let modtype_path s = function
  | Pident id as p -> (
      ...
      with Not_found -> p)
  | Pdot (p, n, pos) -> Pdot (module_path s p, n, pos)
  | Papply (p1, p2) -> fatal_error "Subst.modtype_path"

let type_path s = function
  | Pident id as p -> ( try Tbl.find id s.types with Not_found -> p)
  | Pdot (p, n, pos) -> Pdot (module_path s p, n, pos)
  | Papply (p1, p2) -> fatal_error "Subst.type_path"
...
let rec typexp s ty =
  ...
  | (Tvar _ | Tunivar _) as desc ->
      if s.for_saving || ty.id < 0 then (
        ...
        (* when adding a module to the environment *)
        if ty.level < generic_level then ty.level <- min ty.level s.nongen_level;
        ty)
  | Tsubst ty -> ty
            ...
            Tobject
              ( typexp s t1,
                ref
                  (match !name with
                  | None -> None
                  | Some (p, tl) -> Some (type_path s p, List.map (typexp s) tl))
              )
        | Tfield (m, k, t1, t2)
  ...
  let decl =
    {
      ...
      cty_new =
        (match decl.cty_new with None -> None | Some ty -> Some (typexp s ty));
...
let rec modtype s = function
  | Mty_ident p as mty -> (
      ...
      | Papply (p1, p2) -> fatal_error "Subst.modtype")
  | Mty_signature sg -> Mty_signature (signature s sg)
  | Mty_functor (id, arg, res) ->
      let id' = Ident.rename id in
      Mty_functor
        ( id',
          may_map (modtype s) arg,
...
and signature_component s comp newid =
  match comp with
  | Sig_value (id, d) -> Sig_value (newid, value_description s d)
  | Sig_type (id, d, rs) -> Sig_type (newid, type_declaration s d, rs)
  | Sig_typext (id, ext, es) ->
      Sig_typext (newid, extension_constructor s ext, es)
  | Sig_module (id, d, rs) -> Sig_module (newid, module_declaration s d, rs)
  | Sig_modtype (id, d) -> Sig_modtype (newid, modtype_declaration s d)
  | Sig_class (id, d, rs) -> Sig_class (newid, class_declaration s d, rs)
