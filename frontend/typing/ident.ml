...
let rec find_name name = function
  ...
  | Node (l, k, r, _) ->
      ...
      if c = 0 then k.data else find_name name (if c < 0 then l else r)

let rec get_all = function None -> [] | Some k -> k.data :: get_all k.previous

let rec find_all name = function
  | Empty -> []
  | Node (l, k, r, _) ->
      let c = compare name k.ident.name in
      if c = 0 then k.data :: get_all k.previous
