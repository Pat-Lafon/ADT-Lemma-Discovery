...
module Stdlib = struct
  module String = struct
    ...
    let split s ~on =
      ...
      let rec split1 res i =
        ...
        else if is_separator s.[i] then split1 res (i + 1)
        else split2 res i (i + 1)
      and split2 res i j =
        if j >= String.length s then String.sub s i (j - i) :: res
        else if is_separator s.[j] then
  ...
  module List = struct
    ...
    let filter_map f l =
      let rec aux acc l =
        ...
        | [] -> List.rev acc
        | h :: t -> (
            match f h with None -> aux acc t | Some v -> aux (v :: acc) t)
      in
      aux [] l
  ...
  module Option = struct
    ...
    let fold f a b = match a with None -> b | Some a -> f a b

    let value_default f ~default a =
      match a with None -> default | Some a -> f a
  end
end
...
let split s c =
  ...
  let rec iter pos to_rev =
    if pos = len then List.rev ("" :: to_rev)
      ...
      match try Some (String.index_from s pos c) with Not_found -> None with
      | Some pos2 ->
...
module Color = struct
   ...
   *     mark_open_stag=(mark_open_stag ~or_else:functions.mark_open_tag);
   *     mark_close_stag=(mark_close_tag ~or_else:functions.mark_close_tag);
   *   } in
