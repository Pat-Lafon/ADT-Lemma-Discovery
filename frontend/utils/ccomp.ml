...
let quote_files lst =
  ...
  if String.length s >= 4096 && Sys.os_type = "Win32" then
    build_diversion quoted
  else s
...
let compile_file name =
  ...
  let exit =
    command
      (Printf.sprintf "%s -c %s %s %s %s %s%s"
         (match !Clflags.c_compiler with
         | Some cc -> cc
         | None ->
             if !Clflags.native_code then Config.native_c_compiler
         ...
         *)
         pipe)
  in
  if pipe <> "" then display_msvc_output file name;
  exit
...
let create_archive archive file_list =
  ...
  | _ ->
      ...
      let r1 =
        command
          (Printf.sprintf "%s rc %s %s" Config.ar quoted_archive
             (quote_files file_list))
      in
      if r1 <> 0 || String.length Config.ranlib = 0 then r1
      else command (Config.ranlib ^ " " ^ quoted_archive)

let expand_libname name =
  if String.length name < 2 || String.sub name 0 2 <> "-l" then name
  else
    let libname =
      "lib" ^ String.sub name 2 (String.length name - 2) ^ Config.ext_lib
    in
    try Misc.find_in_path !Config.load_path libname with Not_found -> libname
...
let call_linker mode output_name files extra =
  let cmd =
    if mode = Partial then
      let l_prefix =
        match Config.ccomp_type with "msvc" -> "/libpath:" | _ -> "-L"
      in
      Printf.sprintf "%s%s %s %s %s" Config.native_pack_linker
