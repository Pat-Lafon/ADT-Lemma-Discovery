...
module Impmap = struct
  ...
  let reduce f =
    ...
    | `LeftisthpTree f -> (
        ( "Leftisthp.tree",
          function
          | [ V.I r; V.I x; V.TI a; V.TI b ] ->
              handle (fun () -> [ V.TI (f r x a b) ])
          | _ -> raise @@ InterExn "bad prog" ))
    | `LeftisthpMakeTree f -> (
        ( "Leftisthp.make_tree",
          function
    ...
    | `Merge f -> (
        ( "merge",
          function
          | [ V.TI tree1; V.TI tree2 ] ->
              handle (fun () -> [ V.TI (f tree1 tree2) ])
          | _ -> raise @@ InterExn "bad prog" ))
    | `RbsetLeaf f -> (
        ( "Rbset.leaf",
          function
    ...
    | `RbsetTree f -> (
        ( "Rbset.tree",
          function
          | [ V.B r; V.TB a; V.I x; V.TB b ] ->
              handle (fun () -> [ V.TB (f r a x b) ])
          | _ -> raise @@ InterExn "bad prog" ))
    | `Balance f -> (
        ( "balance",
          function
          | [ V.B r; V.TB tree1; V.I x; V.TB tree2 ] ->
              handle (fun () -> [ V.TB (f r tree1 x tree2) ])
          | _ -> raise @@ InterExn "bad prog" ))
    | `SplayhpLeaf f -> (
        ( "Splayhp.leaf",
          function
    ...
    | `SplayhpNode f -> (
        ( "Splayhp.node",
          function
          | [ V.T left; V.I x; V.T right ] ->
              handle (fun () -> [ V.T (f left x right) ])
          | _ -> raise @@ InterExn "bad prog" ))
    | `Partition f -> (
        ( "partition",
          function
    ...
    | `Ins f -> (
        ( "ins",
          function
          | [ V.I default; V.L i; V.I a; V.T m ] ->
              handle (fun () -> [ V.T (f default i a m) ])
          | _ -> raise @@ InterExn "bad prog" ))
    | `UnbsetLeaf f -> (
        ( "Unbset.leaf",
          function
  ...
  let l_to_map l =
    List.fold_left
      (fun m (name, f) -> StrMap.add name f m)
...
let longident_to_string ld =
  ...
  in
  match x with None -> raise @@ InterExn "longident_to_string" | Some x -> x
...
module TenvEngine = struct
  ...
  let make_tenv signame sigtps tmap funcm = { signame; sigtps; tmap; funcm }
  ...
  let renew_raw_funcm tenv raw_funcm =
    let funcm =
      StrMap.map
        ...
        raw_funcm
    in
    { signame = tenv.signame; sigtps = tenv.sigtps; tmap = tenv.tmap; funcm }
  ...
  let get_type tp =
    ...
    | Ptyp_constr (const, tps) ->
        if List.length tps != 0 then raise @@ InterExn "not imp get_type"
        else type_reduction (longident_to_string const.txt)
  ...
  type if_new = MustNew | MustOld | NoRequirement
  ...
  let register_tmap tenv tvar if_new =
     ...
     * printf "====\n"; *)
    let oldtp =
      match (StrMap.find_opt tenv name, if_new) with
    ...
    | Some tp, None -> (StrMap.add name tp tenv, (tp, name))
    | Some tp1, Some tp2 ->
  ...
  let register tenv tvar if_new =
    ...
    ( { signame = tenv.signame; sigtps = tenv.sigtps; tmap; funcm = tenv.funcm },
      tvar )
...
let expr_to_se (tenv : TenvEngine.t) expr =
  let aux ld tpopt =
    ...
    | Some tp ->
        let tenv, tvar =
          TenvEngine.register tenv (Some tp, name) TenvEngine.NoRequirement
        in
        ([], SE.from_tpedvar tvar, tenv)
    | None -> (
        match StrMap.find_opt tenv.funcm name with
        | Some (t1, t2) ->
            if List.length t1 == 0 && List.length t2 == 1 then
        ...
        | _ ->
            let tenv, tvar =
              TenvEngine.register tenv (None, name) TenvEngine.NoRequirement
            in
            ([], SE.from_tpedvar tvar, tenv))
  in
  match expr.pexp_desc with
...
type termi_status = TermiRaiseExn | TermiNormal of Vc.t * TenvEngine.t

let body_vc_gen client_name (init_tenv : TenvEngine.t) asst expr =
  let tps_to_ses =
    List.fold_left
      (fun r tp -> r @ [ SE.from_tpedvar (tp, T.universal_auto_name tp) ])
      []
  in
  let handle_let vb tenv =
    let tenv, leftvars =
      TenvEngine.register_pattern tenv vb.pvb_pat TenvEngine.MustNew
    in
  ...
  let handle_match_args match_args tenv =
    let rec aux e =
      ...
      | Pexp_ident ld ->
          let _, tvar =
            TenvEngine.register tenv
              (None, longident_to_string ld.txt)
              TenvEngine.MustOld
          in
          [ tvar ]
  ...
  let solve_function_name funcname funcm =
    ...
    | _ ->
        let intps, outtps =
          StrMap.find
            (sprintf "map: body_vc_gen Pexp_apply(%s)" funcname)
            funcm funcname
        in
        (funcname, intps, outtps)
  in
  let rec handle_varaible expr target tenv =
    if List.length target != 1 then raise @@ InterExn "body_vc_gen:Pexp_ident"
    else
      let c, arg1, tenv = expr_to_se tenv expr in
      let c' = Helper.poly_eq [ arg1; List.nth target 0 ] in
      TermiNormal ((match c with [] -> c' | _ -> Vc.And (c @ [ c' ])), tenv)
  ...
  and aux expr target tenv =
    ...
    | Pexp_tuple es, Some target ->
        if List.length target != List.length es then
          raise @@ InterExn "body_vc_gen: Tuple length does not macthed"
    ...
    | Pexp_constant (Pconst_integer (istr, None)), Some target ->
        TermiNormal
          ( Helper.poly_eq
              [
                List.nth target 0;
                SE.Literal (T.Int, SE.L.Int (int_of_string istr));
              ],
            tenv )
          ...
          List.fold_left
            (fun status vb ->
              ...
              | Some (tenv, cs) -> (
                  ...
                  match
                    aux let_body
                      (Some (List.map SE.from_tpedvar new_target))
                      tenv
                  with
                  | TermiRaiseExn -> None
                  | TermiNormal (c, tenv) -> Some (tenv, cs @ [ c ])))
    ...
    | Pexp_apply (func, args), target -> (
        ...
        if String.equal funcname "raise" then TermiRaiseExn
        else
          let funcname, intps, outtps =
            solve_function_name funcname tenv.funcm
          in
          let to_se arg tp tenv =
            ...
            | Pexp_constant (Pconst_integer (istr, None)) ->
                Some
                  ([], SE.Literal (T.Int, SE.L.Int (int_of_string istr)), tenv)
          ...
          | Some (tenv, cs, args) ->
              let target =
                match target with
                | Some target -> target
                | None -> tps_to_ses outtps
              in
              let spec_c =
                if String.equal client_name funcname then
                  match asst with
                  | NoPre (postname, _) -> Vc.SpecApply (postname, args @ target)
                  | HasPre (prename, _, postname, _) ->
                      Vc.Implies
                        ( Vc.SpecApply (prename, args @ target),
                          Vc.SpecApply (postname, args @ target) )
                else Vc.SpecApply (funcname, args @ target)
              in
    ...
    | Pexp_ifthenelse (e1, e2, Some e3), target -> (
        ...
        | TermiNormal (Vc.And [ c1 ], _) -> (
            ...
            | TermiNormal (c2, _), TermiRaiseExn ->
                TermiNormal (Vc.Implies (c1, c2), tenv)
            | TermiRaiseExn, TermiNormal (c3, _) ->
                TermiNormal (Vc.Implies (Vc.Not c1, c3), tenv)
            | TermiNormal (c2, _), TermiNormal (c3, _) ->
                TermiNormal (Vc.Ite (c1, c2, c3), tenv))
        | TermiNormal (_, _) ->
            failwith "condition should be a single application")
    | Pexp_ifthenelse (_, _, None), _ ->
        raise @@ InterExn "no else branch in ite"
    | Pexp_match (case_target, cases), target -> (
        let case_targets =
          List.map SE.from_tpedvar @@ handle_match_args case_target tenv
        in
        let cs =
          List.filter_map
            (fun case ->
            ...
            cases
        in
        match cs with [] -> TermiRaiseExn | _ -> TermiNormal (Vc.Or cs, tenv))
    | _ ->
        raise
        @@ InterExn
             (sprintf "not imp body_vc_gen:%s ~> %s"
  ...
  in
  let _, client_outtps =
    StrMap.find "map: body_vc_gen client_name" init_tenv.funcm client_name
  in
...
let structure_to_signagture struc =
  ...
  | Pstr_modtype m -> (
      ...
      | Some mt -> (
          ...
          | Pmty_signature signature -> (mname, signature)
...
let layout_funcm funcm =
  StrMap.iter
    (fun name (argtps, rettp) ->
      printf "val %s(%s) => (%s)\n" name
        (List.to_string T.layout argtps)
        (List.to_string T.layout rettp))
    funcm

let parse_source asts =
  if List.length asts != 3 then raise @@ InterExn "source wrong format"
  else
    let signature = List.nth asts 0 in
    let clienttp = List.nth asts 1 in
    let client = List.nth asts 2 in
    let (signame, tpnames), (fnames, funcm) =
      signature_to_functypes @@ structure_to_signagture signature
    in
    (* let _ =
     *   printf "solve_functype\n";
     *   Pprintast.core_type Format.std_formatter (structure_to_vd clienttp).pval_type;
     *   raise @@ InterExn "end" in *)
    let client_name, (intp, outtp) =
      vd_to_tpedvars None @@ structure_to_vd clienttp
    in
...
let parse_func_update_tenv tenv expr =
  ...
  let tenv, rawargs =
    List.fold_left
      (fun (tenv, tvars) pattern ->
        let tenv, tvar =
          TenvEngine.register_pattern tenv pattern TenvEngine.MustNew
        in
        (tenv, tvars @ tvar))
      (tenv, []) rawargs
  in
...
let parse_propositional_term tenv expr =
  let expr_to_se x = match expr_to_se tenv x with _, x, _ -> x in
  let rec handle_logic func args =
    ...
    | "not", [ e1 ] -> E.Not (aux e1)
    | "==", [ e1; e2 ] ->
        E.Atom (SE.Op (T.Bool, "==", [ expr_to_se e1; expr_to_se e2 ]))
    | "!=", [ e1; e2 ] ->
        E.Not (E.Atom (SE.Op (T.Bool, "==", [ expr_to_se e1; expr_to_se e2 ])))
    | "<=", [ e1; e2 ] ->
        E.Atom (SE.Op (T.Bool, "<=", [ expr_to_se e1; expr_to_se e2 ]))
    | ">=", [ e1; e2 ] ->
        E.Atom (SE.Op (T.Bool, ">=", [ expr_to_se e1; expr_to_se e2 ]))
...
let parse_assertion client_name init_tenv argtps asts =
  ...
  let get_assertion p =
    ...
    | Pstr_value (_, [ value_binding ]) ->
        ...
        let qvs =
          List.sublist rawargs (List.length argtps, List.length rawargs)
        in
  ...
  let preds, asst =
    ...
    else raise @@ InterExn "assertions wrong format"
  in
  let spectab =
    match asst with
...
let make_holes funcnames funcmap imp_map =
  let aux funcname =
    ...
    let imp =
      StrMap.find (sprintf "make_hole::imp_map(%s)" funcname) imp_map funcname
    in
    let open Helper in
...
let trans (source, assertion) =
  let client_name, (signame, tpnames), fnames, raw_funcm, (intp, outtp), client
      =
    parse_source source
  in
  let init_tenv =
    TenvEngine.make_tenv signame tpnames StrMap.empty StrMap.empty
  in
  ...
  let preds, asst, spectab =
    parse_assertion client_name init_tenv (intp @ outtp) assertion
  in
