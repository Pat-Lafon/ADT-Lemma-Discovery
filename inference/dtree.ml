module type Dtree = sig
  ...
  val fvtab_eq :
    (bool list, label) Hashtbl.t -> (bool list, label) Hashtbl.t -> unit
...
module Dtree : Dtree = struct
  ...
  let exec (dt : feature t) m : bool =
    let rec aux = function
      ...
      | Node (feature, l, r) -> if F.exec feature m then aux l else aux r
    in
    aux dt
  ...
  let dt_summary dt fset =
    ...
    let rec aux idx =
      ...
      let _ =
        if exec_vector dt fset fvec then posnum := !posnum + 1
  ...
  let classify_ fset samples =
    let dt = FastDT.make_dt ~samples ~max_d:50 in
  ...
  let classify_hash fset htab is_pos =
    ...
    let _ =
      Hashtbl.iter
        (fun f v ->
          let _ =
            if is_pos v then Array.set samples !iter (true, Array.of_list f)
  ...
  let two_dt f fset dt1 dt2 =
    ...
    let rec aux idx =
      ...
      let _ =
        if f dt1_b dt2_b then Hashtbl.add ftab fvec Pos
        else Hashtbl.add ftab fvec Neg
      in
      match next idx with None -> () | Some idx -> aux idx
    in
    aux 0;
