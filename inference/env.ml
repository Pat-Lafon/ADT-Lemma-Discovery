...
type flow = { pre_flow : Ast.t; applied_args_map : SE.t list list StrMap.t }

open Yojson.Basic

let encode_applied_args args =
  `List (List.map (fun args -> `List (List.map SE.encode args)) args)

let decode_applied_args json =
  decode_list "decode_applied_args"
    (decode_list "decode_applied_args" SE.decode)
    json
(* function
 * | `List l ->
 *   List.map (fun j ->
...
let encode_flow flow =
  ...
  let applied_args_map_j =
    StrMap.fold
      (fun name sell r ->
        `Assoc
          [ ("name", `String name); ("applied_args", encode_applied_args sell) ]
        :: r)
      flow.applied_args_map []
  in
  `Assoc
...
let decode_flow json =
  ...
  let m =
    ...
    @@ decode_list "decode_flow"
         ...
         j
  in
  { pre_flow = json |> member "pre_flow" |> Ast.decode; applied_args_map = m }
...
let decode_vc json =
  let open Util in
  {
    ...
    multi_pre =
      json |> member "multi_pre" |> decode_list "decode_vc" decode_flow;
    ...
    outputs =
      json |> member "outputs" |> decode_list "decode_vc" T.tpedvar_decode;
    prog = (fun _ -> None);
  }
...
let decode_sr json =
  let decoder = function `Int i -> i | _ -> raise @@ InterExn "decoder_sr" in
  let open Util in
  {
...
let fv_encode fv =
  ...
  `String
    (String.init (Array.length fv) (fun i ->
         if Array.get fv i then 't' else 'f'))
...
let stat_init ?(interval = 300.0) env =
  let p, n, m =
    Hashtbl.fold
      (fun _ label (p, n, m) ->
        match label with
...
let encode_stat stat =
  `Assoc
    [
      ...
      ( "num_weakening_every_interval",
        `List (List.map (fun x -> `Int x) !(stat.num_weakening_every_interval))
      );
...
type consistent_stat = { consist_list : consistent_stat_once list ref }

let init_consistent_stat () = { consist_list = ref [] }

let encode_consistent_stat stat =
  `Assoc
    [
      ( "consist_list",
        `List (List.map encode_consistent_stat_once !(stat.consist_list)) );
    ]
...
let decode_infer_result json =
  ...
  let preds =
    json |> member "preds" |> decode_list "decode_infer_result" to_string
  in
...
let decode_single_infer_result json =
  let open Util in
  let preds =
    json |> member "preds" |> decode_list "decode_single_infer_result" to_string
  in
