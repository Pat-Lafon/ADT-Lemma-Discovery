module type Randomgen = sig
  val gen :
    chooses:int list -> num:int -> tp:Tp.Tp.t -> bound:int -> Pred.Value.t list

  val gen_tpvars :
    tpvars:(Tp.Tp.t * string) list ->
    num:int ->
    fv_num:int ->
    bound:int ->
    int list * Pred.Value.t list list
end
...
module Randomgen = struct
               ...
               | n ->
                   frequency
                     [
                       (1, oneofl [ LabeledTree.Leaf ]);
                       ( 3,
                         map4 node gen (oneofl chooses)
                           (self (n - 1))
                           (self (n - 1)) );
                     ]))
    in
    unique_gen tree_gen num
      (LabeledTree.eq (fun x y -> x == y) (fun x y -> x == y))

  let randomgen_labeled_treei chooses bound num =
    List.map
      (fun t -> LabeledTree.from_tree 0 t)
      (randomgen_tree chooses bound num)

  let randomgen_labeled_treeb chooses bound num =
    List.map
      (fun t -> LabeledTree.from_tree true t)
      (randomgen_tree chooses bound num)
  ...
  let gen ~chooses ~num ~tp ~bound =
    ...
    | T.IntTreeI ->
        List.map (fun l -> V.TI l) @@ randomgen_labeled_treei chooses num bound
    | T.IntTreeB ->
        List.map (fun l -> V.TB l) @@ randomgen_labeled_treeb chooses num bound
  ...
  let gen_tpvars ~tpvars ~num ~fv_num ~bound =
    let intvars, dtvars, bvars =
      List.fold_lefti
        (fun (intvars, dtvars, bvars) idx (tp, _) ->
          if T.is_dt tp then (intvars, (tp, idx) :: dtvars, bvars)
          else
            match tp with
    ...
    let bsamples =
      List.map (fun (tp, idx) -> (idx, gen ~chooses ~num ~tp ~bound)) bvars
    in
    let intsamples =
      match intvars with
      | (_, idx) :: t ->
          (idx, [ V.I 0 ])
          :: List.map (fun (tp, idx) -> (idx, gen ~chooses ~num ~tp ~bound)) t
      | [] -> []
    in
    let dtsamples =
      List.map (fun (tp, idx) -> (idx, gen ~chooses ~num ~tp ~bound)) dtvars
    in
