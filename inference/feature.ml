module type Feature = sig
  ...
  val make_set_from_preds_max :
    ?basicpreds:string list ->
    string list ->
    (Tp.Tp.t * string) list ->
    (Tp.Tp.t * string) list ->
    set
...
module Feature : Feature = struct
  ...
  let exec feature m =
    ...
    | Bo x -> (
        match find x with V.B b -> b | _ -> raise @@ InterExn "exec_feature")
  ...
  let layout = function
    ...
    | Base (op, a, b) -> sprintf "%s %s %s" a op b
    | Bo x -> x
  ...
  let make_set_from_preds_max ?(basicpreds = [ "==" ]) preds args qv =
    let bargs, args =
      List.partition
        (fun (tp, _) -> match tp with T.Bool -> true | _ -> false)
        args
    in
    let bfeatures = List.map (fun (_, name) -> Bo name) bargs in
    let dts, basics = List.partition (fun (tp, _) -> T.is_dt tp) args in
    let make_eq_features elems =
      List.map (fun (a, b) -> Base ("==", a, b))
      @@ List.remove_duplicates (fun (a, b) (a', b') ->
             (a == a' && b = b') || (a == b' && b == a'))
    ...
    let make_pr_features preds dt elems =
      ...
      in
      List.fold_left
        (fun r info -> if T.eq info.P.dttp dttp then r @ aux info else r)
        []
        (List.map P.find_pred_info_by_name preds)
    in
    let pr_features =
      List.fold_left
        (fun featureset dt -> featureset @ make_pr_features preds dt qv)
        [] dts
    in
  ...
  let make_set vars =
    let variable_split (dts, elems, bs) (tp, name) =
      if T.is_dt tp then ((tp, name) :: dts, elems, bs)
      else
        match tp with
    ...
    in
    let pr_features =
      List.fold_left (fun r v -> r @ make_pr_features v elems) [] dts
    in
  ...
  let layout_set (set : set) =
    List.fold_left
      (fun r feature -> sprintf "%s [%s]" r (layout feature))
      "" set
  ...
  let make_target (tp, name) elems =
    let elems =
      List.map
        (fun (tp, name) ->
          match tp with T.Int -> name | _ -> raise @@ UndefExn "make_target")
        elems
    in
    ...
    in
    if T.is_dt tp then List.map (aux name elems) (P.tp_to_preds tp)
    else
      match tp with
      | T.Int ->
  ...
  let encode = function
    ...
    | Bo a -> `Assoc [ ("fd", `String "Bo"); ("a", `String a) ]
  ...
  let decode json =
    ...
    if String.equal fd "Pr" then
      ...
      let args =
        json |> member "args" |> decode_list "decode feature" to_string
      in
      let args' =
        json |> member "args'" |> decode_list "decode feature" to_string
      in
