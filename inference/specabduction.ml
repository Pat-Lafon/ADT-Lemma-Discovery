module SpecAbduction = struct
  ...
  type label = MultiPos | MultiMayNeg | MultiMayNegCache
  ...
  let is_pos = function MultiPos -> true | _ -> false

  let sort_singles_by_fset l =
    let l' =
      List.sort
        (fun a b -> compare (List.length a.Env.fset) (List.length b.Env.fset))
        l
    in
    let _ =
      printf "sorted %s\n" @@ List.to_string (fun a -> a.Env.hole.name) l'
    in
    l'
  ...
  let get_inputs hole =
    ...
    if len > 2 && Char.equal (String.get hole.name (len - 2)) '#' then
      if Char.equal (String.get hole.name (len - 1)) 't' then
        (Some true, hole.args)
      else (Some false, hole.args)
    else
  ...
  let handle_snum snum = match snum with None -> () | Some i -> sver := SV2 i
        ...
        (fun c args_value ->
          let m =
            List.fold_left
              ...
              StrMap.empty
              (List.combine hole.args args_value)
          in
          if Epr.forallformula_exec env.abduciable m then c else c + 1)
        0 samples
    in
    c
  ...
  let sampling hole imp env =
    ...
    let samples =
      R.gens ~chooses:default_range ~num ~tps:inputtps ~bound:samplebound
    in
    let samples =
      List.filter_map
        (fun input ->
    ...
    let s =
      ...
      | SV1 _ ->
          ...
          let qvsamples =
            List.choose_list_list (List.map (fun _ -> r) env.qv)
          in
    ...
    let extract_fv (args_value, qv_value) =
      ...
      let m =
        List.fold_left
          (fun m ((_, name), v) -> StrMap.add name v m)
          m
          (List.combine env.qv qv_value)
      in
      if Epr.exec current_epr m then None
      else
  ...
  let name_qv qv_num =
    List.map (fun n -> (T.Int, n))
    @@ List.init qv_num (fun i -> sprintf "u_%i" i)
  ...
  let init_env mii pre spectable preds numX holel =
    let holes, imps =
      List.fold_left
        (fun (m, imp_m) (hole, imp) ->
          (StrMap.add hole.name hole m, StrMap.add hole.name imp imp_m))
    ...
    let multi_pre =
      List.map
        (fun pre ->
          let applied_args_map =
            StrMap.mapi (fun name _ -> Ast.get_app_args pre name) holes
          in
          let applied_args_map =
            StrMap.filter
              (fun _ r -> match r with [] -> false | _ -> true)
              applied_args_map
          in
          { Env.pre_flow = pre; Env.applied_args_map })
        pre
    in
    let spec_envs =
      StrMap.map (fun hole -> init_spec_env hole preds numX) holes
    in
    let spectable =
      StrMap.fold
        (fun specname spec_env m ->
          let hole = StrMap.find "init_env" holes specname in
          StrMap.update specname
            (fun spec ->
              match spec with
    ...
    in
    { vc; holes; imps; spec_envs }

  let learn_in_spec_env env =
    let dt, dtidx =
      if Hashtbl.length env.fvtab == 0 then (D.T, D.T)
      else D.classify_hash env.fset env.fvtab is_pos
    in
  ...
  let update_env_spectable env =
    let spectable' =
      StrMap.fold
        (fun specname spec_env spectable ->
          ...
          StrMap.update specname
            ...
            spectable)
        env.spec_envs env.vc.Env.spectable
    in
  ...
  let is_verified = function Verified -> true | _ -> false
  let is_cannot_gather = function CannotGather -> true | _ -> false
  ...
  let model_to_instance ctx vc model version =
    ...
    let handle_input (tp, name) pred =
      ...
      let args =
        List.map (List.map (fun i -> SE.Literal (T.Int, SE.L.Int i)))
        @@ List.choose_n qvrange info.P.num_int
      in
      let atoms =
        List.map
          (fun args -> Epr.Atom (SE.Op (T.Bool, pred, dtse :: args)))
          args
      in
      let constr =
        Epr.And
    ...
    let inputs_instances =
      List.map
        (fun (tp, name) ->
          match tp with
          | T.Bool ->
              [ V.B (S.get_pred model (S.Z3aux.tpedvar_to_z3 ctx (tp, name))) ]
          | T.Int -> [ V.I (S.get_int_name ctx model name) ]
          | _ ->
              let c = handle_input (tp, name) in
              let samples =
                R.gen_one ~chooses:qvrange ~num:50 ~tp ~bound:samplebound
              in
              ...
              let samples =
                List.filter_map
                  (fun v -> if Epr.exec c (makem v) then Some v else None)
                  samples
              in
              (* let _ = List.iter (fun v ->
               *     printf "instance: %s;\n" (V.layout v)
    ...
    let inputs_instances =
      ...
      @@ List.map (List.remove_duplicates V.eq) inputs_instances
    in
    let ms =
      List.filter_map
        (fun vs ->
          ...
          | Some outputs ->
              ...
              let m =
                List.fold_left
                  (fun m ((_, name), v) -> StrMap.add name v m)
                  m
                  (List.combine vc.inputs vs)
              in
              let m =
                List.fold_left
                  (fun m ((_, name), v) -> StrMap.add name v m)
                  m
                  (List.combine vc.outputs outputs)
              in
              (* let _ = StrMap.iter (fun name v ->
               *     printf "%s -> %s; " name (V.layout v)
               *   ) m in
               * let _ = printf "\n" in *)
              if Ast.exec vc.post vc.spectable m then None else Some m)
        inputs_instances
    in
  ...
  let inplace_verify_and_gather_fv ctx env flow stat_once =
    let handle_model smt_query model version =
      ...
      let qvrange = S.Z3aux.get_preds_interp model version in
      match
        inplace_gather_fv ctx model smt_query qvrange env flow stat_once
      with
      | None -> Gathered
      | Some pre ->
    ...
    | S.Timeout ->
        raise
        @@ InterExn
             (Printf.sprintf "[%s]inplace_verify_and_gather_fv time out!"
                (SZ.layout_imp_version version))
    | S.SmtSat model -> handle_model smt_query model version
  ...
  let refinement_loop ctx env cstat_once =
    ...
    let rec neg_refine_loop env =
      ...
      let res =
        List.map
          (fun flow -> inplace_verify_and_gather_fv ctx env flow cstat_once)
          env.vc.Env.multi_pre
      in
      if List.for_all is_verified res then NRFNewEnv env
      else if List.exists is_real_cex res then
        ...
        in
        NRFCex cexs
      else if List.exists is_cannot_gather res then NRFIncreaseHyp
      else
    ...
    let rec pos_refine_loop env =
      ...
      let spec_envs' =
        StrMap.mapi
          (fun specname spec_env ->
            ...
            let _ =
              total_sample_pos_num := !total_sample_pos_num + pos_sample_num
            in
      ...
      in
      let _ =
        cstat_once.Env.num_fv_of_samples :=
          !total_pos_num + !(cstat_once.Env.num_fv_of_samples)
      in
      let _ =
        cstat_once.Env.num_pos_sample_violate_spec :=
          !total_sample_pos_num + !(cstat_once.Env.num_pos_sample_violate_spec)
      in
      if !total_pos_num > 0 then
        let env = { env with spec_envs = spec_envs' } in
        let env = update_env_spectable env in
  ...
  let make_single_abd_env vc_env spec_env hole preds uniform_qv_num =
    ...
    let m =
      List.fold_lefti
        (fun m i f ->
          let i' =
            List.find_index "make_single_abd_env" (fun f' -> F.eq f f') fset
          in
          IntMap.add i i' m)
        IntMap.empty spec_env.fset
    in
        ...
        let _ =
          Hashtbl.iter
            (fun vec label ->
              ...
              | MultiMayNegCache ->
                  raise @@ InterExn "never happen in make_single_abd_env")
            spec_env.fvtab
        in
        {
          Env.init_spec = spec;
  ...
  let consistent_solution ctx benchname mii pres spectable holel preds startX =
    ...
    let rec search_hyp numX =
      if numX >= max_qv then
        ...
        let result, delta_time =
          time (fun _ -> refinement_loop ctx env stat_once)
        in
        let _ = stat_once.run_time := delta_time in
        let _ =
          cstat.Env.consist_list := !(cstat.Env.consist_list) @ [ stat_once ]
        in
        match result with
  ...
  let merge_envs total_env single_envs_arr =
    Array.fold_left
      (fun total_env single_env ->
        let spec, _ =
          merge_current single_env.Env.current single_env.Env.fset
        in
        let abduciable = D.to_epr spec in
        let abduciable = Epr.simplify_dt_result abduciable in
        let spec = (single_env.hole.args, (single_env.qv, abduciable)) in
        let _ =
          printf "merged [%s]:\n%s\n" single_env.hole.name
            (Ast.layout_spec spec)
        in
        let new_spectable =
          StrMap.update single_env.Env.hole.name
            (fun v ->
              match v with
  ...
  let epr_exec_fv epr fset fv =
    ...
    let _ =
      List.iter (fun (f, b) -> Hashtbl.add m f b) (List.combine fset fv)
    in
    ...
    let rec aux body =
      ...
      | Atom _ as feature -> (
          ...
          | None ->
              raise
              @@ InterExn
                   (sprintf "spec_exec_fv:%s\n" (pretty_layout_epr feature))
          | Some b -> b)
      | Implies (p1, p2) -> if aux p1 then aux p2 else true
      | Ite (p1, p2, p3) -> if aux p1 then aux p2 else aux p3
      | Not body -> not (aux body)
      | And ps ->
          List.fold_left (fun r body -> if r then aux body else false) true ps
      | Or ps ->
          List.fold_left (fun r body -> if r then true else aux body) false ps
  ...
  let operate_epr f fset epr1 epr2 =
    ...
    let rec aux idx =
      ...
      let _ =
        if f dt1_b dt2_b then
          let _ = pos_counter := !pos_counter + 1 in
          Hashtbl.add ftab fvec D.Pos
        else Hashtbl.add ftab fvec D.Neg
      in
      match next idx with None -> () | Some idx -> aux idx
    in
    let _ = aux 0 in
    let result =
      Epr.simplify_dt_result @@ D.to_epr @@ fst
      @@ D.classify_hash fset ftab D.is_pos
    in
    (!pos_counter, result)
  ...
  let smt_merge_epr ctx preds args init addition =
    ...
    let learn fvtab =
       ...
       *   ) fvtab in *)
      let dt, dtidx =
        if Hashtbl.length fvtab == 0 then (D.T, D.T)
        else D.classify_hash fset fvtab D.is_pos
      in
      let abduciable = D.to_epr dt in
      (dt, dtidx, abduciable)
    in
    let raw_init_body =
      ...
      | [ u ] ->
          Epr.And
            (List.map
               (fun u' -> Epr.subst init_body [ snd u ] [ SE.from_tpedvar u' ])
               addition_qv)
      | [ _; _ ] -> init_body
    ...
    let gather posq fvtab =
      ...
      let q =
        if posq then
          Epr.And [ Epr.Or [ raw_init_body; addition_body ]; Epr.Not result ]
        else
          Epr.And [ Epr.Not (Epr.Or [ raw_init_body; addition_body ]); result ]
      in
    ...
    let rec aux fvtab =
      ...
      if if_already_pos && if_already_neg then fvtab else aux fvtab
    in
    ...
    let p, n =
      Hashtbl.fold
        (fun _ label (p, n) ->
          match label with D.Pos -> (p + 1, n) | _ -> (p, n + 1))
        fvtab (0, 0)
    in
  ...
  let smt_merge_spectable ctx consistentfilename resultfilename output =
    let _, consistentresult =
      Env.decode_infer_result @@ Yojson.Basic.from_file consistentfilename
    in
    let preds, result =
      ...
      StrMap.mapi
        (fun name (args, spec) ->
          ...
          | qv, Epr.Or [ _; addition ] ->
              let _, init =
                StrMap.find "smt_merge_spectable" consistentresult name
              in
              let c, result' =
                smt_merge_epr ctx preds args init (qv, addition)
              in
        ...
        result
    in
    let _ = Hashtbl.iter (fun name c -> printf "%s.#fev+: %i\n" name c) cm in
    Yojson.Basic.to_file output (Env.encode_infer_result (preds, m))

  let merge_spectable spectable preds =
    StrMap.map (fun spec -> snd @@ merge_spec spec preds) spectable

  let try_tmp tmpfile =
    let _, envs = Env.decode_weakening @@ Yojson.Basic.from_file tmpfile in
    let t_env =
      List.find "try_tmp" (fun env -> String.equal env.Env.hole.name "t") envs
    in
    let p, n =
      Hashtbl.fold
        (fun _ label (p, n) ->
    ...
    let _ = printf "p:%i n:%i\n" p n in
    let _, dtidx =
      if Hashtbl.length t_env.fvtab == 0 then (D.T, D.T)
      else D.classify_hash t_env.fset t_env.fvtab D.is_pos
    in
  ...
  let verify_flow ctx vc flow =
    let build_smt_query version =
      Ast.to_z3 ctx
        ...
        vc.Env.spectable version vc.Env.vars
    in
    let version = S.Z3aux.V2 in
    let smt_query = build_smt_query version in
    let _ = printf "%s\n" (Expr.to_string smt_query) in
    ...
    | S.Timeout ->
        ...
        raise
        @@ InterExn
             (Printf.sprintf "[%s]verify_flow time out!"
                (SZ.layout_imp_version version))
    | S.SmtSat _ -> false
  ...
  let save_result name preds names vc =
     ...
     *   ) result in *)
    let _ =
      Yojson.Basic.to_file name (Env.encode_infer_result (preds, result))
    in
    ()
  ...
  let weakening ctx benchname vc single_envs time_bound =
    ...
    let rec aux total_env idx =
      if idx >= Array.length single_envs_arr then total_env
      else
        ...
        | Env.NewMaxed (total_env, single_env') ->
            let _ = Array.set single_envs_arr idx single_env' in
            let _ =
              Env.save_stat benchname single_env'.hole.name time_bound stat
            in
            aux total_env (idx + 1)
    ...
    let _ =
      save_result
        (benchname ^ "_bound_maximal.json")
        total_env.preds names total_env
    in
    let midfile = benchname ^ "_" ^ "bound.json" in
    let _ =
      Yojson.Basic.to_file midfile
        (Env.encode_weakening (total_env, Array.to_list single_envs_arr))
    in
    ...
    let _ =
      save_result
        (benchname ^ "_oracle_maximal.json")
        total_env.preds names total_env
    in
  ...
  let find_weakened_model benchname ctx mii pre spectable =
              ...
              (fun p ->
                ...
                | Ast.SpecApply (name', args) ->
                    if List.exists (String.equal name') names then
                      Some (Ast.SpecApply (bound_prefix ^ name', args))
                    else None
    ...
    let consistent_result =
      try Some (Yojson.Basic.from_file consistent_file) with _ -> None
    in
    let bound_result =
      try Some (Yojson.Basic.from_file bound_file) with _ -> None
    in
    ...
    | Some r -> (
        ...
        | Some r ->
            ...
            let pres =
              List.map Ast.merge_and @@ Ast.to_dnf @@ Ast.eliminate_cond_one pre
            in
            ...
            let res =
              ...
              | S.SmtUnsat -> "Max"
              | S.Timeout -> "Timeout"
              | S.SmtSat _ -> sprintf "%.1f" (delta_time *. 1000.0)
            in
  ...
  let result benchname assertions spectable holel preds =
    ...
    let assertion =
      ...
      | [ a ] ->
          sprintf "assertion:\n%s\n"
            (Ast.layout_spec (StrMap.find "result" spectable a))
      | [ prea; posta ] ->
          sprintf "assertion:\n%s\n=>\n%s\n"
            (Ast.layout_spec (StrMap.find "result" spectable prea))
            (Ast.layout_spec (StrMap.find "result" spectable posta))
      | _ -> raise @@ InterExn "never happen in result"
    in
    let resultfilename =
      sprintf "_remote_result/_%s/_bound_maximal.json" benchname
    in
  ...
  let do_consistent ?(snum = None) ?(uniform_qv_num = 2) benchname ctx mii pre
      spectable holel preds startX =
    ...
    let pres =
      List.sort
        (fun a b -> compare (Ast.conj_length b) (Ast.conj_length a))
        pres
    in
    let _ =
    ...
    | CRFinalEnv env ->
        ...
        let single_envs =
          List.map
            (fun specname ->
              ...
              let single_env =
                make_single_abd_env env.vc spec_env target_hole preds
                  uniform_qv_num
              in
              single_env)
            names
        ...
        let _ =
          Yojson.Basic.to_file midfile
            (Env.encode_weakening (env.vc, single_envs))
        in
  ...
  let do_weakening ctx benchname time_bound =
    ...
    let total_env, single_envs =
      Env.decode_weakening @@ Yojson.Basic.from_file midfile
    in
    let _ = printf "flow\n" in
    let _ =
      List.map
        (fun fl -> printf "flow:%s\n" (Ast.layout fl.Env.pre_flow))
        total_env.multi_pre
    in
    weakening ctx benchname total_env single_envs time_bound

  let multi_infer ?(snum = None) ?(uniform_qv_num = 2) benchname ctx mii pre
      spectable holel preds startX =
    let _ =
      do_consistent ~snum ~uniform_qv_num benchname ctx mii pre spectable holel
        preds startX
    in
  ...
  let do_full ?(snum = None) ?(uniform_qv_num = 2) benchname ctx mii pre
      spectable holel preds startX time_bound =
    let e =
      do_consistent ~snum ~uniform_qv_num benchname ctx mii pre spectable holel
        preds startX
    in
    match e with Cex _ -> e | _ -> do_weakening ctx benchname time_bound
end
