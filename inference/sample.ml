module type RealSample = sig
  ...
  type t = { names : (Tp.Tp.t * string) list; values : value list list }
...
module type FeatureVector = sig
  type t = { feature_set : Feature.Feature.set; vecs : bool list list }

  type data = {
    dfeature_set : Feature.Feature.set;
    labeled_vecs : (bool * bool list) list;
  }

  type sides = Single of data | Double of data * data
...
module FeatureVector = struct
  ...
  let of_real_samples feature_set { names; values } =
    let make_env names value =
      List.fold_left
        (fun m ((_, name), v) -> StrMap.add name v m)
        StrMap.empty (List.combine names value)
    in
    let ss =
      List.map
        (fun value ->
  ...
  let remove_field { feature_set; vecs } target =
    ...
    let indicator =
      Array.init (Array.length farr) (fun i ->
          let op', x' = extract_feature farr.(i) in
          String.equal op op' && x == x')
    in
    let feature_set' =
      List.filter_mapi
        (fun i feature -> if indicator.(i) then None else Some feature)
        feature_set
    in
    let vecs' =
      List.map
        (fun vec ->
          List.filter_mapi
            (fun i b -> if indicator.(i) then None else Some b)
            vec)
        vecs
    in
    ...
    let _ =
      List.iter
        (fun (y, bv) ->
          ...
          | Some Neg -> if y then Hashtbl.replace bv_tbl bv Unclear else ()
          | None -> Hashtbl.add bv_tbl bv (if y then Pos else Neg))
        (List.combine y feature_vectors.vecs)
    in
    let by_value v =
      Hashtbl.fold
        (fun k v' r -> if label_eq (v, v') then k :: r else r)
        bv_tbl []
    in
    let pos, neg, unclear = (by_value Pos, by_value Neg, by_value Unclear) in
