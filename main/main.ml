...
let parse sourcefile =
  let structure =
    Pparse.parse_implementation ~tool_name Format.err_formatter sourcefile
  in
...
let init () =
  ...
  let ctx =
    Z3.mk_context
      [ ("model", "true"); ("proof", "false"); ("timeout", "29999") ]
  in
  ctx
...
let start action sourcefile assertionfile outputdir sampling_bound timebound =
  ...
  let mii, vc, holes, preds, spectab, basic_info =
    Translate.trans (source, assertion)
  in
  ...
  let r () =
    ...
    | InferFull, Some snum ->
        SpecAbd.do_full ~snum:(Some snum) ~uniform_qv_num:1 outputdir ctx mii vc
          spectab holes preds 1 timebound
    | InferFull, None ->
        SpecAbd.do_full outputdir ctx mii vc spectab holes preds 1 timebound
    | InferConsistent, Some snum ->
        SpecAbd.do_consistent ~snum:(Some snum) ~uniform_qv_num:1 outputdir ctx
          mii vc spectab holes preds 1
    | InferConsistent, None ->
        SpecAbd.do_consistent outputdir ctx mii vc spectab holes preds 1
...
let fallback_load outputdir =
  ...
  try
    let preds, spectab =
      Env.decode_infer_result (Yojson.Basic.from_file oracle_max_file)
    in
    (false, preds, spectab)
  with _ -> (
    try
      let preds, spectab =
        Env.decode_infer_result (Yojson.Basic.from_file bound_max_file)
      in
      (true, preds, spectab)
    with _ -> raise @@ Failure "cannot find weakened result!")
  ...
  let aux preds spectable name =
    ...
    let () =
      Yojson.Basic.to_file
        (sprintf "_%s/_count_%s.json" dir funcname)
        (`Assoc [ ("cat", `String name); ("num_phi", `Int num_oracle_fv) ])
    in
    printf "%i\n" num_oracle_fv
  in
  try
    ...
    with _ -> aux preds result "bound"
  with _ -> printf "cannot find %s\n" resultfile
...
let spec_to_coq_string funcname subtasknum spec =
  ...
  let make_args =
    List.fold_left (fun str x -> str ^ tpedvar_to_coqstr x) "" (args @ qv)
  in
  let spec_label =
    sprintf "%s_spec %s" funcname
      (List.fold_left (fun str (_, name) -> sprintf "%s %s" str name) "" args)
  in
  let prefix =
    sprintf "Lemma %s_%i %s: (%s) -> %s." funcname subtasknum make_args
      spec_label (Ast.E.layoutcoq body)
  in
  sprintf "%s\n" prefix
...
let to_coq resultfile funcname =
  ...
  let spec =
    StrMap.find (sprintf "to_coq: cannot find (%s)" funcname) spectable funcname
  in
  let args, (qv, body) =
    match spec with
    | args, (qv, Ast.E.Or _) ->
...
let infer_weakening =
  Command.basic ~summary:"weaken inferred specification mapping"
    Command.Let_syntax.(
      ...
      fun () ->
        let ctx = init () in
        try
          let (_ : SpecAbd.multi_infer_result), delta_time =
            time (fun () -> SpecAbd.do_weakening ctx outputdir time_bound)
          in
          eprintf "Weakening Inference Succeeded in %f(s)!\n" delta_time
        with _ -> eprintf "No Consistent Result Found!\n")

let common_parser doc cont =
  Command.basic ~summary:doc
    Command.Let_syntax.(
      ...
      and sampling_bound =
        flag "-sb" (optional int) ~doc:"the sampling number bound"
      and time_bound = flag "-tb" (optional float) ~doc:"the time bound" in
      ...
      ( "consistent",
        common_parser "infer consistent specification mapping"
          (fun
            ...
            ()
          ->
            start InferConsistent sourcefile assertionfile outputdir
              sampling_bound timebound) );
      ( "full",
        common_parser
          "infer consistent specification mapping, then do weakening"
          (fun
            sourcefile
            assertionfile
            assertionfile
...
let show_consistent =
  Command.basic ~summary:"show consistent result(if exists)"
    Command.Let_syntax.(
      ...
      fun () ->
        ...
        try
          let _, spectab =
            Env2.decode_infer_result (Yojson.Basic.from_file consistent_file)
          in
          let () = Ast.print_spectable spectab in
          ()
        with _ -> failwith "cannot find consistent result!")

let show_weakening =
  Command.basic ~summary:"show maximal result(if exists)"
    Command.Let_syntax.(
      let%map_open outputdir = anon ("outputdir" %: string)
      and if_simplify =
        flag "-s" no_arg
          ~doc:" simplify the inferred maximal specification mapping."
      in
      fun () ->
        let if_bound, preds, spectab = fallback_load outputdir in
        let () =
          match if_simplify with
          | true -> Ast.print_spectable (SpecAbd.merge_spectable spectab preds)
          | _ -> Ast.print_spectable spectab
        in
        let () =
          if if_bound then eprintf "This result is maximal under time bound.\n"
...
let show =
  Command.group ~summary:"show"
    [ ("consistent", show_consistent); ("weakening", show_weakening) ]

let diff =
  Command.basic
    ~summary:
      "timed: calculate the time needed for the SMT solver to find a sample \
       allowed by aweakened solution but not the initial one"
    Command.Let_syntax.(
      let%map_open sourcefile = anon ("sourcefile" %: regular_file)
      and assertionfile = anon ("assertionfile" %: regular_file)
      and outputdir = anon ("outputdir" %: string) in
      fun () -> time_d sourcefile assertionfile outputdir)

let count =
  Command.basic
    ~summary:
      "|𝜙+|: count the total positive feature vectors in the space of \
       weakenings, notice it may cost several minutes. For the blue entries, \
       it will takes serveal hours. "
    Command.Let_syntax.(
      let%map_open outputdir = anon ("outputdir" %: string)
      and funcname = anon ("funcname" %: string) in
      fun () -> count_phi outputdir funcname)
...
let command =
  Command.group
    ~summary:"Data-Driven Abductive Inference of Library Specifications"
    [
      ("infer", infer);
      ("diff", diff);
      ("count", count);
